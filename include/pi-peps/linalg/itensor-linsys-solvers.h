...
namespace itensor {
  ...
  struct LinSysSolver {
    virtual void solve(ITensor& /*A*/,
                       ITensor& /*B*/,
                       ITensor& /*X*/,
                       Args const& /*args*/) const;

    virtual void solve(MatRefc<Real> const& /*A*/,
                       VecRef<Real> const& /*B*/,
                       VecRef<Real> const& /*X*/,
                       Args const& /*args*/) const {
    ...
    virtual void solve(MatRefc<Cplx> const& /*A*/,
                       VecRef<Cplx> const& /*B*/,
                       VecRef<Cplx> const& /*X*/,
                       Args const& /*args*/) const {
      // To be overloaded by derived classes
  ...
  void linsystem(ITensorT<I> A,
    ...
    for (const auto& i : B.inds()) {
      if (hasindex(A, i)) {
        ...
        throw std::runtime_error("[linsystem] A and B indices do not match");
      }
    }

    // Find matching indices of A and X
    if (not X)
      throw std::runtime_error("[linsystem] X has no indices");
    std::vector<I> oi;
    oi.reserve(rank(X));
    // TODO
    if (args.defined("dpl")) {
      ...
      if (dbg)
        std::cout << "[linsystem] provided dpl: " << dpl
                  << " Ordering indices according to B" << std::endl;
      for (const auto& i : B.inds()) {
        auto tmpi = prime(i, dpl);
        if (hasindex(A, prime(i, dpl))) {
          oi.emplace_back(tmpi);
        } else {
          throw std::runtime_error("[linsystem] A and X indices do not match");
        }
      }
    } else {
  ...
  template <typename I>
  void linsystemRank2(ITensorT<I>& A,
                      ITensorT<I>& B,
                      ITensorT<I>& X,
                      LinSysSolver const& solver,
                      Args const& args);
  ...
  template <
    ...
    class VecX,
    class =
      stdx::require<hasMatRange<MatA>, hasVecRange<VecB>, hasVecRange<VecX>>>
  void linsystemRef(MatA&& A,
                    VecB&& B,
                    VecX&& X,
                    LinSysSolver const& solver,
                    Args const& args);

  template <class MatA, class VecB, class VecX, class>
  void linsystemRef(MatA&& A,
                    VecB&& B,
                    VecX&& X,
                    LinSysSolver const& solver,
  ...
  template <typename T>
  void linsystemMatVec(MatRefc<T> const& A,
                       VecRef<T> const& D,
                       VecRef<T> const& V,
                       LinSysSolver const& solver,
                       Args const& args);
