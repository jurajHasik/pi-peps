...
//
  ...
  // SOLVE LINEAR SYSTEM Ax = b for x (Direct Solver by LU Decomposition for
  // General A)
  void F77NAME(zgesv)(LAPACK_INT* n,
                      LAPACK_INT* nrhs,
                      LAPACK_COMPLEX* a,
                      LAPACK_INT* lda,
                      LAPACK_INT* ipiv,
  ...
  void F77NAME(dgesv)(LAPACK_INT* n,
                      LAPACK_INT* nrhs,
                      double* a,
                      LAPACK_INT* lda,
                      LAPACK_INT* ipiv,
  ...
  // SOLVE LINEAR SYSTEM Ax = b for x (Direct Solver by Cholesky Decomposition
  // for Symmetric-Positive-Definite A)
  void F77NAME(zposv)(char* uplo,
                      LAPACK_INT* n,
                      LAPACK_INT* nrhs,
                      LAPACK_COMPLEX* a,
                      LAPACK_INT* lda,
  ...
  LAPACK_INT
  zgesv_wrapper(
    LAPACK_INT n,     // rank of square matrix A
    ...
    Cplx* b)          // matrix b
  {
    ...
    LAPACK_INT info = 0;
    F77NAME(zgesv)
    (&n, &nrhs, cpA.data(), &n, ipiv.data(), pb, &n, &info);  // cpb.data()
    return info;
  }
    ...
    LAPACK_REAL* b)        // matrix b
  {
    ...
    LAPACK_INT info = 0;
    F77NAME(dgesv)
    (&n, &nrhs, cpA.data(), &n, ipiv.data(), b, &n, &info);  // cpb.data()
  ...
  struct CholeskySolver : LinSysSolver {
    void solve(MatRefc<Real> const& A,
               VecRef<Real> const& B,
               VecRef<Real> const& X,
               Args const& args) const override {
      auto dbg = args.getBool("dbg", false);
      if (dbg)
        std::cout << "[CholeskySolver::solve<Real>] called" << std::endl;
      ...
      if (info == 0)
        std::copy(B.data(), B.data() + Mr, X.data());
      else
        throw std::runtime_error("CholeskySolver: error info: " +
                                 std::to_string(info));
    }

    void solve(MatRefc<Cplx> const& A,
               VecRef<Cplx> const& B,
               VecRef<Cplx> const& X,
               Args const& args) const override {
      auto dbg = args.getBool("dbg", false);
      if (dbg)
        std::cout << "[CholeskySolver::solve<Cplx>] called" << std::endl;
