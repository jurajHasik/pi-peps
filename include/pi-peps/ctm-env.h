...
class CtmEnv {
 ...
 public:
  ...
  enum CORNER { LU = 1, RU = 2, RD = 3, LD = 4 };
  ...
  typedef enum NORMALIZATION { NORM_BLE, NORM_PTN } normalization_type;
  ...
  itensor::Index const& tauxByVertex(int direction,
                                     Vertex const& v,
                                     int dir) const {
    return itaux[direction].at(p_cluster->vertexToId(v))[dir];
  }

  std::vector<itensor::Index> envIndPair(std::string const& id0,
                                         int i0,
  ...
  itensor::ITensor DContractEnv(std::string const& id0,
                                int i0,
                                std::string const& id1,
                                int i1) const {
  ...
  // 0|1 => C_LU|C_RU
  // 3|2 => C_LD|C_RD
  std::vector<itensor::Index> envIndCornerPair(std::string const& id,
                                               int ci) const {
    int i1 = ci * 2;
    int i2 = (ci * 2 + 7) % 8;
  ...
  // LEGACY pi-peps/ctm-cluster-io.h )
  ...
  void init(INIT_ENV initEnvType, bool isComplex = false, bool dbg = false);
  ...
  void build_halves_V2(DIRECTION direction,
                       Vertex const& v,
                       itensor::ITensor& H,
                       itensor::ITensor& Ht) const;
  ...
  // void normalizeBLE_ctmStep(char ctmMove, int col, int row, bool dbg =
  // false);
