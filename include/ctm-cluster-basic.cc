...
static std::unique_ptr<Cluster> Cluster_1x1_A::create(nlohmann::json & json_cluster) {
...
struct Cluster_2x2_ABBA : Cluster {
    ...
    Cluster_2x2_ABBA(int ad, int pd) : Cluster(2,1,ad,pd) {
        siteIds = { "A", "B" };
        SI = { {"A",0}, {"B",1} };
    ...
    static std::unique_ptr<Cluster> create(nlohmann::json & json_cluster) {
        ...
        string::std init_type = json_cluster["initBy"].get<std::string>();
