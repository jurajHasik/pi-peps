...
class ModelFactory {
	public:
		using TCreateMethod = std::function<std::unique_ptr<Model>(nlohmann::json & json_model)>;
		...
		std::unique_ptr<Model> create(std::string const& name, nlohmann::json & json_model);
