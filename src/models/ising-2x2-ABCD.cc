...
namespace itensor {
  ...
  MPO_2site getMPO2s_Ising_2site(double tau, double J, double h) {
    ...
    h12 +=
      -J * (2 * SU2_getSpinOp(SU2_S_Z, s1) * 2 * SU2_getSpinOp(SU2_S_Z, s2));
    h12 += -h * ((SU2_getSpinOp(SU2_S_P, s1) + SU2_getSpinOp(SU2_S_M, s1)) *
                   delta(s2, s2p) +
                 delta(s1, s1p) *
                   (SU2_getSpinOp(SU2_S_P, s2) + SU2_getSpinOp(SU2_S_M, s2)));
  ...
  MPO_3site getMPO3s_Ising_3site(double tau, double J, double h) {
    ...
    // STEP1 define exact U_123 = exp(-J(Sz_1.Sz_2 + Sz_2.Sz_3) -
    // h(Sx_1+Sx_2+Sx_3))
    ITensor h123 = ITensor(s1, s2, s3, s1p, s2p, s3p);
    h123 += -J *
            (2 * SU2_getSpinOp(SU2_S_Z, s1) * 2 * SU2_getSpinOp(SU2_S_Z, s2)) *
            delta(s3, s3p);
    h123 += -J * delta(s1, s1p) *
            (2 * SU2_getSpinOp(SU2_S_Z, s2) * 2 * SU2_getSpinOp(SU2_S_Z, s3));
    h123 +=
      -h * (((SU2_getSpinOp(SU2_S_P, s1) + SU2_getSpinOp(SU2_S_M, s1)) *
             delta(s2, s2p)) *
              delta(s3, s3p) +
            (delta(s1, s1p) *
             (SU2_getSpinOp(SU2_S_P, s2) + SU2_getSpinOp(SU2_S_M, s2))) *
              delta(s3, s3p) +
            delta(s1, s1p) * (delta(s2, s2p) * (SU2_getSpinOp(SU2_S_P, s3) +
                                                SU2_getSpinOp(SU2_S_M, s3))));
  ...
  void IsingModel_2x2_ABCD::computeAndWriteObservables(EVBuilder const& ev,
    ...
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(0, 0),
                                      Vertex(1, 0)));  // AB
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(0, 0),
                                      Vertex(0, 1)));  // AC
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(1, 0),
                                      Vertex(1, 1)));  // BD
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(0, 1),
                                      Vertex(1, 1)));  // CD
    ...
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(0, 1),
                                      Vertex(0, 2)));  // CA
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(1, 1),
                                      Vertex(1, 2)));  // DB
    evNN.push_back(4.0 * ev.eval2Smpo(EVBuilder::OP2S_SZSZ, Vertex(1, 1),
                                      Vertex(2, 1)));  // DC
  ...
  std::unique_ptr<Model> IsingModel_2x2_ABCD::create(
  ...
  std::unique_ptr<Engine> IsingModel_2x2_ABCD::buildEngine(
    nlohmann::json& json_model) {
    double arg_J1 = json_model["J1"].get<double>();
    double arg_h = json_model["h"].get<double>();
    ...
    if (arg_fuGateSeq == "2SITE") {
      ...
      pe->td.gateMPO.push_back(
        getMPO2s_Ising_2site(arg_tau, arg_J1, arg_h / 4.0));
      ...
      // complete sequence of gates acts applies Sx term 12 times on each site
      // h->h/12.0

      pe->td.gateMPO.push_back(
        getMPO3s_Ising_3site(arg_tau, arg_J1 / 4.0, arg_h / 12.0));
      ...
      std::cout << "[IsingModel_2x2_ABCD] Unsupported gate sequence: "
                << arg_fuGateSeq << std::endl;
      exit(EXIT_FAILURE);
    }
  ...
  std::unique_ptr<Engine> IsingModel_2x2_AB::buildEngine(
    nlohmann::json& json_model) {
    double arg_J1 = json_model["J1"].get<double>();
    double arg_h = json_model["h"].get<double>();
    double arg_tau = json_model["tau"].get<double>();
    ...
    if (arg_fuGateSeq == "2SITE") {
      ...
      std::cout << "[IsingModel_2x2_AB] Unsupported gate sequence: "
                << arg_fuGateSeq << std::endl;
      exit(EXIT_FAILURE);
    }
