...
MPO_3site getMPO3s_J1J2(double tau, double J1, double J2, double del) {
    ...
    ITensor nnS1S2 = J1 * SU2_getSpinOp(SU2_S_Z, s1) * SU2_getSpinOp(SU2_S_Z, s2)
        + J1*0.5*( SU2_getSpinOp(SU2_S_P, s1) * SU2_getSpinOp(SU2_S_M, s2)
...
J1J2Model_2x2_ABCD::J1J2Model_2x2_ABCD(double arg_J1, double arg_J2,
    double arg_del) : J1(arg_J1), J2(arg_J2), del(arg_del) {}
...
std::unique_ptr<Model> J1J2Model_2x2_ABCD::create(nlohmann::json & json_model) {
    
    double arg_J1 = json_model["J1"].get<double>();
    double arg_J2 = json_model["J2"].get<double>();
    double arg_del = 0.0;
    
    return std::unique_ptr<Model>(new J1J2Model_2x2_ABCD(arg_J1, arg_J2, arg_del));
}

std::unique_ptr<Engine> J1J2Model_2x2_ABCD::buildEngine(nlohmann::json & json_model) {
    ...
    if (arg_fuGateSeq == "SYM1") {
        ...
        std::cout<<"J1J2Model_2x2_ABCD SYM1 ENGINE constructed"<<std::endl;
        ...
        std::cout<<"J1J2Model_2x2_ABCD SYM2 ENGINE constructed"<<std::endl;
        if (arg_symmTrotter) pe->td.symmetrize();
        return std::unique_ptr<Engine>( pe );
    }
