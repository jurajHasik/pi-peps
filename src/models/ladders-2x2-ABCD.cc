...
namespace itensor {
  ...
  MPO_2site getMPO2s_Ladders(double tau, double J) {
    ...
    h12 +=
      J * (SU2_getSpinOp(SU2_S_Z, s1) * SU2_getSpinOp(SU2_S_Z, s2) +
  ...
  MPO_3site getMPO3s_Ladders(double tau, double J, double alpha) {
    ...
    h123 += J *
            (SU2_getSpinOp(SU2_S_Z, s1) * SU2_getSpinOp(SU2_S_Z, s2) +
             0.5 * (SU2_getSpinOp(SU2_S_P, s1) * SU2_getSpinOp(SU2_S_M, s2) +
                    SU2_getSpinOp(SU2_S_M, s1) * SU2_getSpinOp(SU2_S_P, s2))) *
            delta(s3, s3p);
    h123 += alpha * J *
            (SU2_getSpinOp(SU2_S_Z, s2) * SU2_getSpinOp(SU2_S_Z, s3) +
  ...
  OpNS getOP4s_Ladders(double tau, double J, double alpha) {
    ...
    h4 += alpha * J * nnS1S2(s2, s3) * delta(s1, s1p) *
          delta(s4, s4p);  // id1S2S3id4
    h4 += alpha * J * nnS1S2(s1, s4) * delta(s2, s2p) *
  ...
  void LaddersModel_2x2_ABCD::computeAndWriteObservables(EVBuilder const& ev,
    ...
    // write magnetization
    double evMag_avg = 0.;
    evMag_avg = 0.25 * (std::sqrt(ev_sA[0] * ev_sA[0] + ev_sA[1] * ev_sA[2]) +
    ...
    // write Energy
    double energy =
      ((evNN[0] + evNN[1] + evNN[2] + evNN[3] + evNN[4] + evNN[7]) * J +
       (evNN[5] + evNN[6]) * (alpha * J)) /
      4.0;
    output << " " << energy;

    // write components of magentization
    output << " " << ev_sA[0] << " " << 0.5 * (ev_sA[1] + ev_sA[2]) << " "
           << 0.5 * (ev_sA[1] - ev_sA[2]);
    output << " " << ev_sB[0] << " " << 0.5 * (ev_sB[1] + ev_sB[2]) << " "
           << 0.5 * (ev_sB[1] - ev_sB[2]);
    output << " " << ev_sC[0] << " " << 0.5 * (ev_sC[1] + ev_sC[2]) << " "
           << 0.5 * (ev_sC[1] - ev_sC[2]);
    output << " " << ev_sD[0] << " " << 0.5 * (ev_sD[1] + ev_sD[2]) << " "
           << 0.5 * (ev_sD[1] - ev_sD[2]);
  ...
  std::unique_ptr<Model> LaddersModel_2x2_ABCD::create(
  ...
  std::unique_ptr<Engine> LaddersModel_2x2_ABCD::buildEngine(
    nlohmann::json& json_model) {
    double arg_J = json_model["J1"].get<double>();
    double arg_alpha = json_model["alpha"].get<double>();
    ...
    if (arg_fuGateSeq == "2SITE") {
      ...
      pe->td.tgates = {
        ...
        TrotterGate<MPO_2site>(Vertex(1, 1), {Shift(0, 1)},
                               &pe->td.gateMPO[1])};

      std::cout << "LaddersModel_2x2_ABCD 2SITE ENGINE constructed"
                << std::endl;
      if (arg_symmTrotter)
        pe->td.symmetrize();
  ...
  void LaddersModel_4x2_ABCD::computeAndWriteObservables(EVBuilder const& ev,
    ...
    // horizontal
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(0, 0), Vertex(1, 0)));  // A1A2
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(1, 0), Vertex(2, 0)));  // A2A3
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(2, 0), Vertex(3, 0)));  // A3A4
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(3, 0), Vertex(4, 0),
                                true));  // A4A1
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(0, 1), Vertex(1, 1)));  // B1B2
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(1, 1), Vertex(2, 1)));  // B2B3
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(2, 1), Vertex(3, 1)));  // B3B4
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(3, 1), Vertex(4, 1)));  // B4B1

    // vertical
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(0, 0), Vertex(0, 1)));  // A1B1
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(1, 0), Vertex(1, 1)));  // A2B2
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(2, 0), Vertex(2, 1)));  // A3B3
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(3, 0), Vertex(3, 1)));  // A4B4
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(0, 1), Vertex(0, 2)));  // B1A1
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(1, 1), Vertex(1, 2)));  // B2A2
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(2, 1), Vertex(2, 2)));  // B3A3
    evNN.push_back(
      ev.eval2Smpo(EVBuilder::OP2S_SS, Vertex(3, 1), Vertex(3, 2)));  // B4A4
    ...
    double evMag_avg = 0.;
    for (int j = 0; j < 8; j++)
      evMag_avg += (1.0 / 8.0) * std::sqrt(szspsm[j][0] * szspsm[j][0] +
                                           szspsm[j][1] * szspsm[j][1]);
    output << " " << evMag_avg;

    // write Energy
    double energy = 0.0;
    for (int j = 0; j < 12; j++)
      energy += (1.0 / 8.0) * evNN[j] * J;
    for (int j = 12; j < 16; j++)
      energy += (1.0 / 8.0) * evNN[j] * (J * alpha);
    output << " " << energy;
  ...
  std::unique_ptr<Model> LaddersModel_4x2_ABCD::create(
    nlohmann::json& json_model) {
    double arg_J = json_model["J1"].get<double>();
    double arg_alpha = json_model["alpha"].get<double>();
  ...
  std::unique_ptr<Engine> LaddersModel_4x2_ABCD::buildEngine(
    nlohmann::json& json_model) {
    double arg_J = json_model["J1"].get<double>();
    double arg_alpha = json_model["alpha"].get<double>();
    double arg_tau = json_model["tau"].get<double>();
    ...
    if (arg_fuGateSeq == "2SITE") {
      ...
      pe->td.tgates = {
        ...
        TrotterGate<MPO_2site>(Vertex(3, 1), {Shift(0, 1)},
                               &pe->td.gateMPO[1])};

      std::cout << "LaddersModel_4x2_ABCD 2SITE ENGINE constructed"
                << std::endl;
      if (arg_symmTrotter)
        pe->td.symmetrize();
      return std::unique_ptr<Engine>(pe);
