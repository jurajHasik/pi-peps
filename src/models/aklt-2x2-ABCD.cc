...
namespace itensor {
  ...
  ITensor Projector_S2tpS2_S1_v2(Index& s1, Index& s2) {
    ...
    auto tmp_S1S2_3 =
      mapprime(tmp_S1S2 * prime(tmp_S1S2, 1) * prime(tmp_S1S2, 2), 3, 1);
    auto tmp_S1S2_4 = mapprime(
      tmp_S1S2 * prime(tmp_S1S2, 1) * prime(tmp_S1S2, 2) * prime(tmp_S1S2, 3),
      4, 1);

    auto p =
      (1.0 / 14.0) * (tmp_S1S2 + (7.0 / 10.0) * tmp_S1S2_2 +
  ...
  void AKLTModel_2x2_ABCD::computeAndWriteObservables(EVBuilder const& ev,
    ...
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(0, 0),
                                Vertex(1, 0)));  // AB
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(0, 0),
                                Vertex(0, 1)));  // AC
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(1, 0),
                                Vertex(1, 1)));  // BD
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(0, 1),
                                Vertex(1, 1)));  // CD
    ...
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(0, 1),
                                Vertex(0, 2)));  // CA
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(1, 1),
                                Vertex(1, 2)));  // DB
    evNN.push_back(ev.eval2Smpo(EVBuilder::OP2S_AKLT_S2_H, Vertex(1, 1),
                                Vertex(2, 1)));  // DC
  ...
  std::unique_ptr<Model> AKLTModel_2x2_ABCD::create(
    nlohmann::json& json_model) {
    return std::unique_ptr<Model>(new AKLTModel_2x2_ABCD());
  }

  std::unique_ptr<Engine> AKLTModel_2x2_ABCD::buildEngine(
    nlohmann::json& json_model) {
  ...
  void AKLTModel_2x2_AB::computeAndWriteObservables(EVBuilder const& ev,
    ...
    // write magnetization
    double evMag_avg = 0.;
    evMag_avg = 0.5 * (std::sqrt(ev_sA[0] * ev_sA[0] + ev_sA[1] * ev_sA[2]) +
                       std::sqrt(ev_sB[0] * ev_sB[0] + ev_sB[1] * ev_sB[2]));
    output << " " << evMag_avg;
    ...
    output << " " << ev_sA[0] << " " << 0.5 * (ev_sA[1] + ev_sA[2]) << " "
           << 0.5 * (ev_sA[1] - ev_sA[2]) << " " << ev_sB[0] << " "
           << 0.5 * (ev_sB[1] + ev_sB[2]) << " " << 0.5 * (ev_sB[1] - ev_sB[2]);
  ...
  std::unique_ptr<Engine> AKLTModel_2x2_AB::buildEngine(
    nlohmann::json& json_model) {
    double arg_tau = json_model["tau"].get<double>();
    double arg_lambda = json_model["lambda"].get<double>();
