...
namespace itensor {
  ...
  MPO_3site getMPO3s_HB(double tau, double J, double h, double del) {
    ...
    // del:  XY limit -1 <-- 0 (HB)-->--> >>1 Ising limit
    // STEP 1 define exact h_123 = J*(S_1.S_2 + S_2.S_3) + del*(Sz_1Sz_2 +
    // Sz_2Sz_3)
    // - h*(Sz_1+Sz_2+Sz_3)
    ITensor nnS1S2 =
      (J + del) * SU2_getSpinOp(SU2_S_Z, s1) * SU2_getSpinOp(SU2_S_Z, s2) +
      J * 0.5 *
  ...
  OpNS getOP4s_HB(double tau, double J, double h, double del) {
    ...
    h4 += h * SU2_getSpinOp(SU2_S_Z, s1) * delta(s2, s2p) * delta(s3, s3p) *
          delta(s4, s4p);
    h4 += h * delta(s1, s1p) * SU2_getSpinOp(SU2_S_Z, s2) * delta(s3, s3p) *
          delta(s4, s4p);
    h4 += h * delta(s1, s1p) * delta(s2, s2p) * SU2_getSpinOp(SU2_S_Z, s3) *
          delta(s4, s4p);
    h4 += h * delta(s1, s1p) * delta(s2, s2p) * delta(s3, s3p) *
          SU2_getSpinOp(SU2_S_Z, s4);
  ...
  HeisenbergModel_2x2_ABCD::HeisenbergModel_2x2_ABCD(double arg_J,
                                                     double arg_h,
                                                     double arg_del)
  ...
  void HeisenbergModel_2x2_ABCD::computeAndWriteObservables(
    EVBuilder const& ev,
    std::ofstream& output,
    Args& metaInf) {
    ...
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(0, 0),
                                Vertex(0, 1)));  // AC
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(1, 0),
                                Vertex(1, 1)));  // BD
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(0, 1),
                                Vertex(1, 1)));  // CD
    ...
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(0, 1),
                                Vertex(0, 2)));  // CA
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(1, 1),
                                Vertex(1, 2)));  // DB
    evNN.push_back(ev.eval2Smpo(std::make_pair(nnSS.H1, nnSS.H2), Vertex(1, 1),
                                Vertex(2, 1)));  // DC
    ...
    // write Energy
    double energy = 0.25 * ((evNN[0] + evNN[1] + evNN[2] + evNN[3] + evNN[4] +
                             evNN[5] + evNN[6] + evNN[7]) +
  ...
  std::unique_ptr<Model> HeisenbergModel_2x2_ABCD::create(
    ...
    return std::unique_ptr<Model>(
      new HeisenbergModel_2x2_ABCD(arg_J, arg_h, arg_del));
  }

  std::unique_ptr<Engine> HeisenbergModel_2x2_ABCD::buildEngine(
    nlohmann::json& json_model) {
    double arg_J = json_model["J1"].get<double>();
    double arg_h = json_model["h"].get<double>();
    double arg_del = json_model["del"].get<double>();
    ...
    if (arg_fuGateSeq == "2SITE") {
      ...
      std::cout << "HeisenbergModel_2x2_ABCD 2SITE ENGINE constructed"
                << std::endl;
      if (arg_symmTrotter)
        pe->td.symmetrize();
      ...
      pe->td.gateMPO.push_back(
        getMPO3s_HB(arg_tau, arg_J / 4.0, arg_h / 12.0, arg_del / 4.0));
      ...
      std::cout << "HeisenbergModel_2x2_ABCD SYM3 ENGINE constructed"
                << std::endl;
      if (arg_symmTrotter)
        pe->td.symmetrize();
      return std::unique_ptr<Engine>(pe);
      ...
      pe->td.gateMPO.push_back(
        getOP4s_HB(arg_tau, arg_J / 2.0, arg_h / 4.0, arg_del / 2.0));
      ...
      std::cout << "HeisenbergModel_2x2_ABCD 4SITE ENGINE constructed"
                << std::endl;
      if (arg_symmTrotter)
        pe->td.symmetrize();
      return std::unique_ptr<Engine>(pe);
  ...
  // ----- Definition of model base class and its particular instances --
  HeisenbergModel_2x2_AB::HeisenbergModel_2x2_AB(double arg_J,
                                                 double arg_h,
                                                 double arg_del)
    : J(arg_J), h(arg_h), del(arg_del) {}
  ...
  void HeisenbergModel_2x2_AB::computeAndWriteObservables(EVBuilder const& ev,
    ...
    // write magnetization
    double evMag_avg = 0.;
    evMag_avg = 0.5 * (std::sqrt(ev_sA[0] * ev_sA[0] + ev_sA[1] * ev_sA[1]) +
                       std::sqrt(ev_sB[0] * ev_sB[0] + ev_sB[1] * ev_sB[1]));
    output << " " << evMag_avg;
  ...
  std::unique_ptr<Model> HeisenbergModel_2x2_AB::create(
    nlohmann::json& json_model) {
    double arg_J = json_model["J1"].get<double>();
    double arg_h = json_model["h"].get<double>();
    double arg_del = json_model["del"].get<double>();
    ...
    return std::unique_ptr<Model>(
      new HeisenbergModel_2x2_AB(arg_J, arg_h, arg_del));
  }
  ...
  void HeisenbergModel_1x1_A::computeAndWriteObservables(EVBuilder const& ev,
    ...
    ITensor h12 = ITensor(s1, s2, s1p, s2p);
    h12 += (J + del) * SU2_getSpinOp(SU2_S_Z, s1) *
  ...
  //     pe->td.tgates = {
  //       TrotterGate<MPO_2site>(Vertex(0, 0), {Shift(1, 0)},
  //       &pe->td.gateMPO[0]), TrotterGate<MPO_2site>(Vertex(1, 0), {Shift(1,
  //       0)}, &pe->td.gateMPO[0]), TrotterGate<MPO_2site>(Vertex(0, 0),
  //       {Shift(0, 1)}, &pe->td.gateMPO[0]), TrotterGate<MPO_2site>(Vertex(1,
  //       0), {Shift(0, 1)},
  //                              &pe->td.gateMPO[0])};
  ...
  //     return std::unique_ptr<Engine>(pe);
  //   } else {
  //     std::cout << "Unsupported gate sequence: " << arg_fuGateSeq <<
