...
MPO_3site getMPO3s_Ising3Body(double tau, double J1, double J2, double h) {
  ...
  h123 +=
    -J1 *
    (2.0 * SU2_getSpinOp(SU2_S_Z, s1) * 2.0 * SU2_getSpinOp(SU2_S_Z, s2)) *
    delta(s3, s3p);
  h123 += -J1 * delta(s1, s1p) *
          (2.0 * SU2_getSpinOp(SU2_S_Z, s2) * 2.0 * SU2_getSpinOp(SU2_S_Z, s3));
  h123 +=
  ...
  h123 += -J2 * (2.0 * SU2_getSpinOp(SU2_S_Z, s1) * 2.0 *
                 SU2_getSpinOp(SU2_S_Z, s2) * 2.0 * SU2_getSpinOp(SU2_S_Z, s3));
...
J1QModel::J1QModel(double arg_J1, double arg_Q) : J1(arg_J1), Q(arg_Q) {}
...
void J1QModel::computeAndWriteObservables(EVBuilder const& ev,
  ...
  double avgSSSS = 0.;
  for (unsigned int j = 0; j < evSSSS.size(); j++)
    avgSSSS += evSSSS[j];
  avgSSSS = avgSSSS / 4.0;
  output << " " << avgSSSS;
  ...
  // write magnetization
  double evMag_avg = 0.;
  evMag_avg = 0.25 * (sqrt(ev_sA[0] * ev_sA[0] + ev_sA[1] * ev_sA[1]) +
...
//     return std::unique_ptr<Model>(new Ising3BodyModel(arg_J1, arg_J2,
//     arg_h));
// }
...
//     ptr_model = std::unique_ptr<Model>(new Ising3BodyModel(arg_J1, arg_J2,
//     arg_h));
...
//         gates = {
//             {"A", "B", "D", "C"}, {"D", "C", "A", "B"}, //{"A", "C", "D",
//             "B"}, // (1 AD ABCD)
//             {"B", "A", "C", "D"}, {"C", "D", "B", "A"}, //{"B", "D", "C",
//             "A"}, // (1 BC ABCD)
...
//         gateMPO.push_back( getMPO3s_Ising3Body(arg_tau, arg_J1/4.0, arg_J2,
//         arg_h/12.0) ); ptr_gateMPO = std::vector< MPO_3site * >(16,
//         &(gateMPO[0]) );
...
//         &(gateMPO[0]) );
//     } else {
//         std::cout<<"Unsupported 3-site gate sequence: "<< arg_fuGateSeq <<
...
//         getModel_J1J2(json_model, ptr_model, gateMPO, ptr_gateMPO, gates,
//         gate_auxInds);
//     } else if (arg_modelType == "NNHLadder") {
//         getModel_NNHLadder(json_model, ptr_model, gateMPO, ptr_gateMPO,
//         gates, gate_auxInds);
//     } else if (arg_modelType == "Ising") {
//         getModel_Ising(json_model, ptr_model, gateMPO, ptr_gateMPO, gates,
//         gate_auxInds);
//     } else if (arg_modelType == "Ising3Body") {
//         getModel_Ising3Body(json_model, ptr_model, gateMPO, ptr_gateMPO,
//         gates, gate_auxInds);
//     } else {
