...
Args fullUpdate_ALS2S_IT(MPO_2site const& mpo, Cluster & cls, CtmEnv const& ctmEnv,
	...
	for (int i=0; i<2; i++) phys[i] = cls.mphys.at( tn[i] );
	...
	ITensor eRE;
	{
		...
		plI2 = (plI1  + 1) % 4;
		crI  = plI1 * 10 + plI2;
		pc[0] = ((*iToT.at(plI1)).at(tn[0]) * (*iToC.at(crI)).at(tn[0])) * (*iToT.at(plI2)).at(tn[0]);
		tmp_iToE[0][0] = plI1;
		tmp_iToE[0][1] = plI2;
		...
		plI2 = (plI1  + 1) % 4;
		crI  = plI1 * 10 + plI2;
		pc[2] = ((*iToT.at(plI1)).at(tn[1]) * (*iToC.at(crI)).at(tn[1])) * (*iToT.at(plI2)).at(tn[1]);
		tmp_iToE[1][0] = plI1;
		tmp_iToE[1][1] = plI2;
		...
		if(dbg && (dbgLvl >=3)) for (int i=0; i<4; i++) Print(pc[i]);
		...
		else throw std::runtime_error("[fullUpdate_ALS2S_IT] Invalid gate");
		// 2) find the vertex of tn[0]
		auto v0 = cls.idToV.at(tn[0]);
		auto v1 = v0 + shift;

		pc[0] *= delta(ctmEnv.tauxByVertex(dir0,v0,pl[0]),ctmEnv.tauxByVertex(dir0,v1,pl[1]));
		pc[1] *= delta(ctmEnv.tauxByVertex(dir1,v0,pl[0]),ctmEnv.tauxByVertex(dir1,v1,pl[1]));
	...
	ITensor protoK = (eRE * eA) * delta(cls.AIc(tn[0],pl[0]), cls.AIc(tn[1],pl[1]));
	protoK *= eB;
	...
	if (fuTrialInit) {
		...
		auto tmpT = eA * delta(cls.AIc(tn[0],pl[0]),cls.AIc(tn[1],pl[1])) * eB * tmpOp;
		tmpT.noprime(PHYS);
...
Args fullUpdate_2S(MPO_2site const& mpo, Cluster & cls, CtmEnv const& ctmEnv,
	std::vector<std::string> const& tn, std::vector<int> pl, 
	LinSysSolver const& ls,
	Args const& args) {
	...
	auto NORMPSI = (eRE * eB) * delta(cls.AIc(tn[1],pl[1]),cls.AIc(tn[0],pl[0]));
	NORMPSI *= (prime(conj(eB), AUXLINK, 4) * prime(delta(cls.AIc(tn[1],pl[1]),cls.AIc(tn[0],pl[0])), 4) );
	...
	// Solve for eA*eB
	auto M = (eRE * delta(phys[0], prime(phys[0],4))) * delta(phys[1], prime(phys[1],4));
	auto K = prime(protoK,PHYS,4);

	auto cmb0 = combiner(iQA, iQB, phys[0], phys[1]);
	auto cmb1 = combiner(prime(iQA,4), prime(iQB,4), prime(phys[0],4), prime(phys[1],4));
	M = (cmb0 * M) * cmb1;
	K *= cmb1;
	...
	t_fdistN = 1.0 - 2.0 * sumels(OVERLAP)/std::sqrt(sumels(NORMPSI)*normUPsi) + 1.0;
