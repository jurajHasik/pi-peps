...
void applyH_T1_L_T2(MPO_2site const& mpo2s,
  ...
  ITensor SV_L12;
  spec =
    svd(T1R * L * T2R, T1R, SV_L12, T2R, {"Maxm", auxd, "svCutoff", svCutoff});
  if (dbg) {
    Print(T1R);
    Print(spec);
    Print(T2R);
...
void applyH_T1_L_T2_v2(MPO_2site const& mpo2s,
  ...
  ITensor T1X, T2X, sv1XR, sv2XR;
  auto spec =
    svd((T1 * L) * delta(iT1_L, iL_T2), T1R, sv1XR, T1X, {"Truncate", false});
  if (dbg)
    Print(spec);
  spec =
    svd((T2 * L) * delta(iT1_L, iL_T2), T2R, sv2XR, T2X, {"Truncate", false});
  if (dbg)
    Print(spec);
  T1R = T1R * sv1XR;
  T2R = T2R * sv2XR;
...
void applyH_T1_L_T2_v2_notReduceTensors(MPO_2site const& mpo2s,
  ...
  for (auto const& ind : T1.inds())
    if (ind != iT1_L)
      outer.emplace_back(ind);
  ...
  auto spec =
    svd(T1 * L * T2, T1R, SV_L12, T2R, {"Maxm", iT1_L.m(), "Minm", iT1_L.m()});
  if (dbg) {
    Print(T1R);
    Print(spec);
    Print(T2R);
...
void applyH_123_v1(MPO_3site const& mpo3s,
  ...
  ITensor res = (mT1 * delta(s1, mpo3s.Is1)) * l12 *
                (mT2 * delta(s2, mpo3s.Is2)) * l23 *
...
void applyH_123_v2(MPO_3site const& mpo3s,
  ...
  for (size_t i = 1; i <= n1.m(); ++i)
    tmpD[i - 1] = sv1.real(n1(i), n2(i));
  l12 = diagTensor(tmpD, a1, a2);

  tmpD = std::vector<double>(a3.m(), 0.0);
  for (size_t i = 1; i <= n3.m(); ++i)
    tmpD[i - 1] = sv2.real(n3(i), n4(i));
...
Args simpleUpdate(MPO_2site const& u12,
                  Cluster& cls,
  ...
  for (const auto& lw : cls.siteToWeights.at(tn[0]))
    if (lw.dirs[0] != pl[0])
      tmpT[0] *= getInvDiagT(cls.weights.at(lw.wId));
  ...
  for (int i = 0; i < 2; i++)
    cls.sites[tn[i]] = tmpT[i];

  for (const auto& lw : cls.siteToWeights.at(tn[0]))
    if (lw.dirs[0] == pl[0])
      cls.weights[lw.wId] = l12;

  return diag_data;
}

Args simpleUpdate(MPO_3site const& u123,
                  Cluster& cls,
                  std::vector<std::string> tn,
                  std::vector<int> pl,
                  Args const& args) {
  ...
  for (const auto& lw : cls.siteToWeights.at(tn[2]))
    if (lw.dirs[0] != pl[4])
      tmpT[2] *= getInvDiagT(cls.weights.at(lw.wId));
  ...
  for (const auto& lw : cls.siteToWeights.at(tn[1]))
    if (lw.dirs[0] == pl[3])
      cls.weights[lw.wId] = l23;

  return diag_data;
}

Args simpleUpdate(OpNS const& u12,
                  Cluster& cls,
                  std::vector<std::string> tn,
                  std::vector<int> pl,
                  Args const& args) {
