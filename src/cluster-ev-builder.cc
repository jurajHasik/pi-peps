...
void EVBuilder::TransferOpVecProd_itensor::operator()(ITensor& bT, bool DBG) {
  using DIRECTION = CtmEnv::DIRECTION;
...
double EVBuilder::analyzeBoundaryVariance(Vertex const& v,
  ...
  if (dir == DIRECTION::RIGHT) {
    ...
    bTother *=
      delta(p_ctmEnv->tauxByVertex(DIRECTION::UP, v_other, DIRECTION::RIGHT),
            p_ctmEnv->tauxByVertex(DIRECTION::UP, v, DIRECTION::LEFT));
    auto tmp_del = delta(p_cluster->AIc(v_other, DIRECTION::RIGHT),
                         p_cluster->AIc(v, DIRECTION::LEFT));
    bTother *= tmp_del;
    bTother *= prime(tmp_del, p_cluster->BRAKET_OFFSET);
    bTother *=
      delta(p_ctmEnv->tauxByVertex(DIRECTION::DOWN, v_other, DIRECTION::RIGHT),
    ...
    bTother *=
      delta(p_ctmEnv->tauxByVertex(DIRECTION::LEFT, v_other, DIRECTION::DOWN),
            p_ctmEnv->tauxByVertex(DIRECTION::LEFT, v, DIRECTION::UP));
    auto tmp_del = delta(p_cluster->AIc(v_other, DIRECTION::DOWN),
                         p_cluster->AIc(v, DIRECTION::UP));
    bTother *= tmp_del;
    bTother *= prime(tmp_del, p_cluster->BRAKET_OFFSET);
    bTother *=
      delta(p_ctmEnv->tauxByVertex(DIRECTION::RIGHT, v_other, DIRECTION::DOWN),
...
// double EVBuilder::eval2Smpo_redDenMat2x1(OP_2S op2s, std::pair<int,int> s1,
// std::pair<int,int> s2,
//         bool DBG) const
// {
//     auto rdm = redDenMat2x1(s1,s2,DBG);
//     if ( rdm.r() != 4 ) std::cout << "ERROR rdm rank: "<< rdm.r() <<
//     std::endl;
...
//     // Perform some coord manipulation (assumed coords are >= 0) and
//     // s1.first <= s2.second && s1.second <= s2.second
//     if ( (s1.first < 0) || (s1.second < 0) || (s2.first < 0) || (s2.second
//     <0)
//         || (s1.second > s2.second) ) {
...
//         if(DBG) std::cout <<"s1.first > s2.first => s1 defines upper right
//         corner"
//             << std::endl;
//         s1bs2 = false;
//     }
...
//         if(DBG) std::cout <<">>>>> 1) Left edge constructed <<<<<"<<
//         std::endl; if(DBG) Print(tN);
...
//                 //     // contract Op.first with bra and ket on-site tensors
//                 of site s1
//                 //     auto mpo_s1 = getTOT(Op.first, cls.cToS.at(s), 0,
//                 DBG);
//                 //     if(DBG) std::cout <<"Op.first inserted at ["<< s.first
//                 <<","
//                 //         << s.second <<"] -> "<< cls.cToS.at(s) <<
//                 std::endl;
//                 //     tN = tN * prime(mpo_s1.mpo[0], HSLINK,
//                 2*(sYdiff-row+s1.second));
//                 // } else if ((col==s2.first) && (row == s2.second)) {
//                 //     // contract Op.second with bra and ket on-site tensors
//                 of site s2
//                 //     auto mpo_s2 = getTOT(Op.second, cls.cToS.at(s), 0,
//                 DBG);
//                 //     if(DBG) std::cout <<"Op.second inserted at ["<<
//                 s.first
//                 //         <<","<< s.second <<"] -> "<< cls.cToS.at(s)
//                 //         << std::endl;
//                 //     tN = tN * prime(mpo_s2.mpo[0], HSLINK,
//                 2*(sYdiff-row+s1.second));
//                 // } else {
//                     tN *= cls.sites.at(cls.cToS.at(s));
//                     tN *= prime(prime(conj(cls.sites.at(cls.cToS.at(s))),
//                     AUXLINK, 4 ), PHYS, 1);
//                 // }
//                 tN *= cmbL3;
...
//         tNR *= prime(prime(conj(cls.sites.at(cls.cToS.at(s))), AUXLINK, 4 ),
//         PHYS, 1);
...
//                 auto cmbT2 = combiner(
//                 prime(cls.aux[cls.SI.at(cls.cToS.at(s))],2),
//                     prime(cls.aux[cls.SI.at(cls.cToS.at(s))],6) );
...
//                 //         << s.second <<"] -> "<< cls.cToS.at(s) <<
//                 std::endl;
//                 //     tN *= prime(mpo_s1.mpo[0], VSLINK,
//                 2*(sXdiff-col+s1.first));
//                 // } else if ((col==s2.first) && (row == s2.second)) {
//                 //     auto mpo_s2 = getTOT(Op.second, cls.cToS.at(s), 0,
//                 DBG);
//                 //     if(DBG) std::cout <<"Op.second inserted at ["<<
...
//         tNB *= prime(prime(conj(cls.sites.at(cls.cToS.at(s))), AUXLINK, 4 ),
//         PHYS, 1);
...
//     //     if(DBG) { std::cout <<"Args: s1["<< s1.first <<","<< s1.second
//     <<"]"<< std::endl;
//     //         std::cout <<"Args: s2["<< s2.first <<","<< s2.second <<"]"<<
//     std::endl; }
...
//     //         std::cout <<"s1["<< s1.first <<","<< s1.second <<"]"<<
//     std::endl;
//     //         std::cout <<"s2["<< s2.first <<","<< s2.second <<"]"<<
//     std::endl; }
...
//     //         if(DBG) std::cout<<"["<< s.first <<","<< row <<"] =>"
//     //             <<"["<< s.first <<","<<
//     //         (s.second + abs(s.second)*cd_f.sizeN) % cd_f.sizeN
//     <<"]"<<std::endl;
...
//     //             s.second = (s.second + abs(s.second)*cd_f.sizeN) %
//     cd_f.sizeN;
...
//     //                     << s.second <<"] -> "<< cls.cToS.at(s) <<
//     std::endl;
//     //                 if(DBG) std::cout <<"HSLINK primeLvl: "<<
//     2*abs(abs(sYdiff)+row-s2.second)
//     //                         << std::endl;
//     //                 tN *= prime(mpo_s1.mpo[0], HSLINK,
//     2*abs(abs(sYdiff)+row-s2.second));
//     //             } else if ((col==s2.first) && (row == s2.second)) {
//     //                 // contract Op.second with bra and ket on-site tensors
//     of site s2
...
//         bool DBG) const
// {
//     return contract2x2Diag1N1(op2s, s1, DBG)/contract2x2Diag1N1(OP2S_Id, s1,
//     DBG);
// }
...
//     auto e_s1 = std::make_pair(s1.first % cls.sizeM, s1.second % cls.sizeN);
//     //diagonal s2 = s1 + [1,-1]
//     auto e_s2 = std::make_pair((s1.first+1) % cls.sizeM,
//     (s1.second-1+cls.sizeN) % cls.sizeN);

//     // sY s2
//     // s1 sX
//     auto sX = std::make_pair((s1.first+1) % cls.sizeM, s1.second %
//     cls.sizeN); auto sY = std::make_pair(s1.first % cls.sizeM,
//     (s1.second-1+cls.sizeN) % cls.sizeN);

//     if(DBG) { std::cout <<"s1: ["<< e_s1.first <<","<< e_s1.second
//     <<"]"<<std::endl; std::cout <<"sX: ["<< sX.first <<","<< sX.second
//     <<"]"<<std::endl; std::cout <<"sY: ["<< sY.first <<","<< sY.second
//     <<"]"<<std::endl; std::cout <<"s2: ["<< e_s2.first <<","<< e_s2.second
//     <<"]"<<std::endl; }
