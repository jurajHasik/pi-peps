...
void initRT_basic(ITensor& rt, std::string INIT_METHOD, Args const& args) {
  if (INIT_METHOD == "RANDOM") {
    ...
    Index a2 =
      (a1.primeLevel() < IOFFSET) ? prime(a1, IOFFSET) : prime(a1, -IOFFSET);
...
ITensor getT(ITensor const& s,
             std::array<Index, 4> const& plToEnv,
             ITensor const& op,
             std::array<const ITensor*, 4> rt,
             bool dbg) {
...
ITensor getT(ITensor const& s,
  ...
  if (op)
    res = (res * delta(pS, pOp)) * op;
  if (dbg)
    Print(res);
  // build bra part
  // apply conjugate of physical operator
  if (!pIcont)
    res.prime(prime(pOp, 1), 1);
  if (op)
    res = res * prime(conj(op), MPOLINK, 4);
  if (dbg)
    Print(res);
  // apply reduction tensors if present to <bra|
  if (rt[1])
    res = res * prime(conj(*rt[1]), 4);
  if (rt[3])
    res = res * prime(conj(*rt[3]), 4);
  if (dbg && (rt[1] || rt[3]))
    Print(res);
...
ITensor getketT(ITensor const& s,
                ITensor const& op,
                std::array<const ITensor*, 2> rt,
                bool dbg) {
  Index aS(noprime(findtype(s, AUXLINK)));
  Index pS(noprime(findtype(s, PHYS)));
  Index pOp = (op) ? noprime(findtype(op, PHYS)) : Index();
  ...
  // apply reduction tensors if present to |ket>
  if (rt[0])
    res = res * noprime(*rt[0], MPOLINK);
  if (rt[1])
    res = res * noprime(*rt[1], MPOLINK);
  if (dbg && (rt[0] || rt[1]))
    Print(res);
  // apply physical operator - acting on physical index
  if (op)
    res = ((res * delta(pS, pOp)) * op) * delta(prime(pOp), pS);
  if (dbg)
    Print(res);
  // reset primeLevel of auxIndices from isometries back to primeLevel
  // of on-site indices
...
Args fullUpdate(MPO_3site const& uJ1J2,
                Cluster& cls,
                CtmEnv const& ctmEnv,
  ...
  if (symmProtoEnv) {
    ...
    if (posDefProtoEnv) {
      ...
      if (dbg && (dbgLvl >= 1)) {
        ...
        dbg_oss << std::scientific << "MAX EV: " << mval << " MIN EV: " << nval
                << std::endl;
        dbg_oss << "RATIO svd_cutoff/negative/all " << countCTF << "/"
  ...
  protoK = (protoK * delta(opPI[0], phys[0])) *
           conj(uJ1J2.H1).prime(uJ1J2.a12, 4 + pl[1]);
  if (dbg && (dbgLvl >= 3))
    Print(protoK);

  protoK = protoK * conj(eD).prime(AUXLINK, 4);
  protoK = (protoK * delta(opPI[2], phys[2])) *
           conj(uJ1J2.H3).prime(uJ1J2.a23, 4 + pl[4]);
  if (dbg && (dbgLvl >= 3))
    Print(protoK);

  protoK = protoK * conj(eB).prime(AUXLINK, 4);
  protoK =
    (protoK * delta(opPI[1], phys[1])) *
  ...
  rt[0] = ITensor(prime(aux[0], pl[1]), prime(uJ1J2.a12, pl[1]),
                  prime(aux[0], pl[1] + IOFFSET));
  rt[1] = ITensor(prime(aux[1], pl[2]), prime(uJ1J2.a12, pl[2]),
                  prime(aux[1], pl[2] + IOFFSET));
  rt[2] = ITensor(prime(aux[1], pl[3]), prime(uJ1J2.a23, pl[3]),
                  prime(aux[1], pl[3] + IOFFSET));
  rt[3] = ITensor(prime(aux[2], pl[4]), prime(uJ1J2.a23, pl[4]),
                  prime(aux[2], pl[4] + IOFFSET));
  // simple initialization routines
  if (rtInitType == "RANDOM" || rtInitType == "DELTA" ||
      rtInitType == "NOISE") {
    for (int i = 0; i <= 3; i++) {
  ...
  // self-consistent initialization - guess isometries between tn[0] and tn[1]
    ...
    ITensor ttemp =
      (protoK *
       delta(prime(uJ1J2.a23, 4 + plRT[2]), prime(uJ1J2.a23, 4 + plRT[3]))) *
      delta(prime(auxRT[2], 4 + plRT[2]), prime(auxRT[3], 4 + plRT[3]));

    auto cmb0 =
      combiner(prime(auxRT[0], 4 + plRT[0]), prime(uJ1J2.a12, 4 + plRT[0]));
    auto cmb1 =
      combiner(prime(auxRT[1], 4 + plRT[1]), prime(uJ1J2.a12, 4 + plRT[1]));
    ...
    rt[0] = (tU * delta(commonIndex(tS, tU),
                        prime(findtype(cmb0, AUXLINK), IOFFSET))) *
            cmb0;
    rt[1] = (tV * delta(commonIndex(tS, tV),
                        prime(findtype(cmb1, AUXLINK), IOFFSET))) *
            cmb1;
    rt[0].prime(-4);
    rt[1].prime(-4);
    ...
    // rt[2]--rt[3]
    ttemp = (protoK * delta(prime(uJ1J2.a12, 4 + plRT[0]),
                            prime(uJ1J2.a12, 4 + plRT[1]))) *
            delta(prime(auxRT[0], 4 + plRT[0]), prime(auxRT[1], 4 + plRT[1]));

    cmb0 =
      combiner(prime(auxRT[2], 4 + plRT[2]), prime(uJ1J2.a23, 4 + plRT[2]));
    cmb1 =
      combiner(prime(auxRT[3], 4 + plRT[3]), prime(uJ1J2.a23, 4 + plRT[3]));
    ...
    std::cout << "Unsupported fu-isometry initialization: " << rtInitType
              << std::endl;
    exit(EXIT_FAILURE);
  }
  ...
  while (not converged) {
    for (int i_rt = 0; i_rt <= 3; i_rt++) {
      ...
      // construct matrix M, which is defined as <psi~|psi~> = rt[r]^dag * M *
      // rt[r]
      if (dbg && (dbgLvl >= 3))
        std::cout << "COMPUTING Matrix M" << std::endl;
      std::array<const itensor::ITensor*, 2>
        rtp;  // holds reduction tensor pointers => rtp
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][0][i] >= 0) ? &rt[RTPM_R[i_rt][0][i]] : NULL;
      ITensor M =
        eRE * getketT(*QS[ord[0]], *mpo[ord[0]], rtp, (dbg && (dbgLvl >= 3)));
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][1][i] >= 0) ? &rt[RTPM_R[i_rt][1][i]] : NULL;
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][2][i] >= 0) ? &rt[RTPM_R[i_rt][2][i]] : NULL;
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][0][i] >= 0) ? &rt[RTPM_R[i_rt][0][i]] : NULL;
      M = M * prime(conj(getketT(*QS[ord[0]], *mpo[ord[0]], rtp,
                                 (dbg && (dbgLvl >= 3)))),
                    AUXLINK, 4);
      M *=
        delta(prime(aux[ord[0]], 4 + pl[2 * ord[0] + (1 - ORD_DIR[i_rt]) / 2]),
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][1][i] >= 0) ? &rt[RTPM_R[i_rt][1][i]] : NULL;
      M = M * (prime(conj(getketT(*QS[ord[1]], *mpo[ord[1]], rtp,
                                  (dbg && (dbgLvl >= 3)))),
                     AUXLINK, 4) *
      ...
      for (int i = 0; i < 2; i++)
        rtp[i] = (RTPM_R[i_rt][2][i] >= 0) ? &rt[RTPM_R[i_rt][2][i]] : NULL;
      M = M * prime(conj(getketT(*QS[ord[2]], *mpo[ord[2]], rtp,
                                 (dbg && (dbgLvl >= 3)))),
                    AUXLINK, 4);

      M.mapprime(MPOLINK, 0, 4 + plRT[i_rt], MPOLINK, plRT[i_rt] + IOFFSET,
                 plRT[i_rt]);
      if (dbg && (dbgLvl >= 3))
        Print(M);
      ...
      if (dbg && (dbgLvl >= 3))
        std::cout << "COMPUTING Vector K" << std::endl;
      ...
      K *= delta(prime(auxRT[current_rt], 100 + 4 + plRT[current_rt]),
                 prime(auxRT[i_rt], 100 + 4 + plRT[i_rt]));
      if (dbg && (dbgLvl >= 3))
        Print(K);
      ...
      if (dbg && (dbgLvl >= 1)) {
        ...
        auto Mdag = conj(M);
        Mdag =
          swapPrime(swapPrime(Mdag, pl[2 * ord[2] + (1 + ORD_DIR[i_rt]) / 2],
                              4 + pl[2 * ord[2] + (1 + ORD_DIR[i_rt]) / 2]),
                    IOFFSET + pl[2 * ord[2] + (1 + ORD_DIR[i_rt]) / 2],
                    4 + IOFFSET + pl[2 * ord[2] + (1 + ORD_DIR[i_rt]) / 2]);
      ...
      auto cmbKp = combiner(prime(K.inds()[0], -4), prime(K.inds()[1], -4),
                            prime(K.inds()[2], -4));
      ...
      // Solve linear system M*rt[r] = K for rt[r] by constructing the
      // pseudoinverse M^dag of M via SVD. Then the solution, which minimizes
      // norm distance |M rt[r] - K| is given by M^dag K = rt[r]
      M = (cmbK * M) * cmbKp;
      // M = M * (1.0 + lambda);
      K = cmbK * K;
      ...
      // symmetrize
      auto Msym =
        0.5 * (M + ((conj(M) * delta(combinedIndex(cmbK),
                                     prime(combinedIndex(cmbKp), 1))) *
                    delta(prime(combinedIndex(cmbK), 1), combinedIndex(cmbKp)))
                     .prime(-1));
      ...
      // Find largest (absolute) EV and in the case of msign * mval < 0,
      // multiply dM by -1 as to set the sign of largest (and consecutive) EV to
      // +1 SYM SOLUTION
      double msign = 1.0;
      double mval = 0.;
      std::vector<double> dM_elems;
      for (int idm = 1; idm <= dM.inds().front().m(); idm++) {
        dM_elems.push_back(
          dM.real(dM.inds().front()(idm), dM.inds().back()(idm)));
        if (std::abs(dM_elems.back()) > mval) {
          mval = std::abs(dM_elems.back());
          msign = dM_elems.back() / mval;
        }
      ...
      // 		std::cout <<
      // regInvSvM.real(regInvSvM.inds().front()(isv),regInvSvM.inds().back()(isv))
      // 		<< std::endl;
      // }
      ...
      // }
      // std::cout << std::endl;
      // for (int i=0; i<=3; i++) std::cout<<" iso["<< i <<"].scale(): "<<
      // rt[i].scale(); std::cout << std::endl;
      ...
      // if (i_rt==3) {
      {
        ...
        tempOLP = (prime(conj(niso), 4) * M) * niso;
        if (rank(tempOLP) > 0)
          std::cout << "ERROR - tempOLP not a scalar" << std::endl;
        ovrlp_val = sumelsC(tempOLP);
        if (isComplex(tempOLP)) {
          std::cout << "NORM is Complex: imag(ovrlp_val)=" << ovrlp_val.imag()
                    << std::endl;
        }
        ...
        ovrlp_val = sumelsC(tempOLP);
        if (isComplex(tempOLP)) {
          std::cout << "OVERLAP is Complex: imag(ovrlp_val)="
    ...
    if (altlstsquares_iter > 1) {
      auto dist_prev = normUPsi + overlaps[overlaps.size() - 6] -
                       2.0 * overlaps[overlaps.size() - 5];
      //- overlaps[overlaps.size()-4];
      auto dist_curr = normUPsi + overlaps[overlaps.size() - 3] -
                       2.0 * overlaps[overlaps.size() - 2];
  ...
  auto newT = getketT(cls.sites.at(tn[0]), uJ1J2.H1, {&rt[0], NULL},
                      (dbg && (dbgLvl >= 3)));
  cls.sites.at(tn[0]) = newT;
  newT = getketT(cls.sites.at(tn[1]), uJ1J2.H2, {&rt[1], &rt[2]},
                 (dbg && (dbgLvl >= 3)));
  cls.sites.at(tn[1]) = newT;
  newT = getketT(cls.sites.at(tn[2]), uJ1J2.H3, {&rt[3], NULL},
                 (dbg && (dbgLvl >= 3)));
  ...
  auto dist1 = normUPsi + overlaps[overlaps.size() - 3] -
               2.0 * overlaps[overlaps.size() - 2];
  ...
  std::string logMinDiag_descriptor =
    "f_init f_final norm(psi')_init norm(psi')_final time[s]";
...
Args fullUpdate_ALS2S_IT(MPO_2site const& mpo,
  ...
  ITensor eRE;
  {
    ...
    plI2 = (plI1 + 1) % 4;
    crI = plI1 * 10 + plI2;
    pc[0] = ((*iToT.at(plI1))[si[0]] * (*iToC.at(crI))[si[0]]) *
            (*iToT.at(plI2))[si[0]];
    tmp_iToE[0][0] = plI1;
    tmp_iToE[0][1] = plI2;
    ...
    plI2 = (plI1 + 1) % 4;
    crI = plI1 * 10 + plI2;
    pc[2] = ((*iToT.at(plI1))[si[1]] * (*iToC.at(crI))[si[1]]) *
            (*iToT.at(plI2))[si[1]];
    tmp_iToE[1][0] = plI1;
    tmp_iToE[1][1] = plI2;
    ...
    {
      auto cmb0 = combiner(prime(aux[0], tmp_iToE[0][0]),
                           prime(aux[0], tmp_iToE[0][0] + 4));
      auto cmb1 = combiner(prime(aux[0], tmp_iToE[0][1]),
                           prime(aux[0], tmp_iToE[0][1] + 4));
      pc[0] = (pc[0] * delta(combinedIndex(cmb0), iToE[0][0])) * cmb0;
      pc[0] = (pc[0] * delta(combinedIndex(cmb1), iToE[0][1])) * cmb1;

      cmb0 = combiner(prime(aux[0], tmp_iToE[0][2]),
                      prime(aux[0], tmp_iToE[0][2] + 4));
      pc[1] = (pc[1] * delta(combinedIndex(cmb0), iToE[0][2])) * cmb0;

      cmb0 = combiner(prime(aux[1], tmp_iToE[1][0]),
                      prime(aux[1], tmp_iToE[1][0] + 4));
      cmb1 = combiner(prime(aux[1], tmp_iToE[1][1]),
                      prime(aux[1], tmp_iToE[1][1] + 4));
      pc[2] = (pc[2] * delta(combinedIndex(cmb0), iToE[1][0])) * cmb0;
      pc[2] = (pc[2] * delta(combinedIndex(cmb1), iToE[1][1])) * cmb1;

      cmb0 = combiner(prime(aux[1], tmp_iToE[1][2]),
                      prime(aux[1], tmp_iToE[1][2] + 4));
    ...
    // Decompose A tensor on which the gate is applied
    {
      ...
      auto maskS = [&machine_eps](Real r) {
        return (r > 10.0 * machine_eps) ? 1.0 : 0.0;
      };
      auto cutoffS = [&machine_eps](Real r) {
        return (r > 10.0 * machine_eps) ? r : 0;
      };
  ...
  ITensor protoK =
    (eRE * eA) * delta(prime(aux[0], pl[0]), prime(aux[1], pl[1]));
  protoK *= eB;
  ...
  if (fuTrialInit) {
    auto SqrtT = [&machine_eps](Real r) {
      return (r > sqrt(10.0 * machine_eps)) ? sqrt(r) : 0;
    };
    ...
    auto tmpT =
      eA * delta(prime(aux[0], pl[0]), prime(aux[1], pl[1])) * eB * tmpOp;
    tmpT.noprime(PHYS);
  ...
  while (not converged) {
    ...
    {
      ITensor M =
        (eRE * eB) * delta(prime(aux[1], pl[1]), prime(aux[0], pl[0]));
      M *= (prime(conj(eB), AUXLINK, 4) *
            prime(delta(prime(aux[1], pl[1]), prime(aux[0], pl[0])), 4));
      ...
      auto cmb1 =
        combiner(prime(iQA, 4), prime(aux[0], pl[0] + 4), prime(phys[0], 4));
      M = (cmb0 * M) * cmb1;
      K *= cmb1;
      eA *= cmb0;
    ...
    // 1) construct matrix M, which is defined as <psi~|psi~> = eB^dag * M * eB
    {
      ...
      M *= (prime(conj(eA), AUXLINK, 4) *
            prime(delta(prime(aux[0], pl[0]), prime(aux[1], pl[1])), 4));
      ...
      auto cmb1 =
        combiner(prime(iQB, 4), prime(aux[1], pl[1] + 4), prime(phys[1], 4));
      M = (cmb0 * M) * cmb1;
      K *= cmb1;
      eB *= cmb0;
