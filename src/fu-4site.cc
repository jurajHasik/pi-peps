...
Args fullUpdate_CG_full4S(OpNS const& uJ1J2,
                          Cluster& cls,
                          CtmEnv const& ctmEnv,
  ...
  std::vector<Index> iQX;
  {
    ...
    for (int i = 0; i < 4; i++) {
      ITensor X(cls.AIc(tn[i], pl[2 * i]), cls.AIc(tn[i], pl[2 * i + 1]),
                phys[i]),
        Q, SV;
  ...
  for (int i = 0; i < 4; i++) {
    ...
    // std::cout<<"Copying rX["<< i <<"] of size: "<< n << " to x["<< i*n
    // <<":"<< (i+1)*n-1<<"]"<< std::endl; PrintData(rX[i]);
  ...
  oss << std::scientific << 0.0 << " " << final_dist << " " << 0.0 << " " << 0.0
      << " " << 0.0 << " " << g4s.inst_normPsi << " "
      << std::chrono::duration_cast<std::chrono::microseconds>(t_end_int -
                                                               t_begin_int)
             .count() /
           1000000.0;
...
FU4SiteGradMin::FU4SiteGradMin(
  ...
  auto l_AIc = [this](std::string id, int dir) {
    return ctmEnv.p_cluster->AIc(id, dir);
  };
  ...
  auto getEdgeCombiners_fromTnAndPl = [this, &getEdgeCombiners](
    ...
    getEdgeCombiners(cmb_v0, cmb_v1, edge_dir, v0, dir_outgoing_s0, v1,
                     dir_ingoing_s1, edge);
  };
  ...
  if (dbg && dbgLvl >= 3) {
    std::cout << "<psi|U^dag U|psi> = " << normUPsi << std::endl;
  }
}
...
Real FU4SiteGradMin::operator()(Vec<Real> const& x, Vec<Real>& grad) {
  ...
  for (int i = 0; i < 4; i++) {
    ...
    auto rawRX =
      subVector(x, i * n, (i + 1) * n - 1);  // reference to a part of x
    // std::cout<<"[operator()] making rawRX["<< n <<"] from slice x["<< i*n
    // <<":"<< (i+1)*n-1<<"]"<< std::endl;
...
std::vector<double> FU4SiteGradMin::func() const {
  ...
  v[1] = sumels(NORMPSI);
  return v;
  // return 1.0 - 2.0 * sumels(OVERLAP)/std::sqrt(sumels(NORMPSI) * normUPsi)
  // + 1.0; // normalized
}
...
void FU4SiteGradMin::gradient(std::vector<ITensor>& grad) {
  ...
  for (int i = 0; i < 4; i++) {
    grad[i] = 0.0 * prime(rX[i], AUXLINK, 4);
  }
  ...
  // compute d<psi'|psi'> contributions
  {
    ...
    for (int i = 0; i < 4; i++) {
      ...
      temp *=
        prime(delta(l_AIc(tn[j], pl[2 * j]), l_AIc(tn[i], pl[2 * i + 1])), 4);
      temp *=
        prime(delta(l_AIc(tn[j], pl[2 * j + 1]), l_AIc(tn[k], pl[2 * k])), 4);
      ...
      // grad[i] = temp * (0.5 * inst_overlap / (std::sqrt(normUPsi *
      // inst_normPsi) * std::abs(inst_normPsi)) );
    }
  }
...
// 				<<". ||psi'> - |psi>|^2 = "<< fret << " Diff: " << std::abs(fret -
// fp)/finit << std::endl; 			return;
// 		}
// 		fp=fret;
...
// 		// }
// 		// if (test < GTOL) {
// 		// 	std::cout << "Frprmn: converged iter="<< its <<". g^2 = "<< test <<
// std::endl;
// 		// 	return p;
// 		// }
...
// 			if (tempT.r() > 0) std::cout <<"ERROR: tempT is not a scalar" <<
// std::endl; 			dgg += sumels(tempT);
// 		}
