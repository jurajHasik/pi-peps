...
OpNS::OpNS(int n, std::string uuid_)
  : uuid(uuid_), nSite(n), siteIds(n), pi(n) {}
...
MPO_3site::MPO_3site(MPO_3site const& op)
  : MpoNS(op),
    H1(mpo.at(0)),
    H2(mpo.at(1)),
    H3(mpo.at(2)),
...
MPO_2site::MPO_2site(MPO_2site const& op)
  : MpoNS(op),
    H1(mpo.at(0)),
    H2(mpo.at(1)),
    Is1(pi.at(0)),
...
// ----- MPOs construction --------------------------------------------
MPO_2site symmMPO2Sdecomp(ITensor const& u12,
                          Index const& s1,
                          Index const& s2,
                          bool dbg) {
  Index s1p = prime(s1);
  ...
  if (dbg)
    PrintData(mpo2s.H1 * mpo2s.H2);

  return mpo2s;
}

MPO_3site symmMPO3Sdecomp(ITensor const& u123,
                          Index const& s1,
                          Index const& s2,
                          Index const& s3,
                          bool dbg) {
