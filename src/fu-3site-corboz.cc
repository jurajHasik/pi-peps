...
Args fu_3s_corboz(MPO_3site const& u123, Cluster & cls, CtmEnv const& ctmEnv,
	...
	protoK = (( protoK * delta(opPI[0],phys[0]) ) * u123.H1) * prime(delta(opPI[0],phys[0]));
	protoK = (( protoK * delta(opPI[1],phys[1]) ) * u123.H2) * prime(delta(opPI[1],phys[1]));
	protoK = (( protoK * delta(opPI[2],phys[2]) ) * u123.H3) * prime(delta(opPI[2],phys[2]));
	protoK.prime(PHYS,-1);
	if(dbg && (dbgLvl >=3)) Print(protoK);

	// <psi|U^dag U|psi>
	auto NORMUPSI = (( protoK * delta(opPI[0],phys[0]) ) * conj(u123.H1)) * prime(delta(opPI[0],phys[0]));
	NORMUPSI = (( NORMUPSI * delta(opPI[1],phys[1]) ) * conj(u123.H2)) * prime(delta(opPI[1],phys[1]));
	NORMUPSI = (( NORMUPSI * delta(opPI[2],phys[2]) ) * conj(u123.H3)) * prime(delta(opPI[2],phys[2]));
	...
	protoK = ( protoK * delta(opPI[0],phys[0]) ) * conj(u123.H1).prime(u123.a12, 4+pl[1]);
	if(dbg && (dbgLvl >=3)) Print(protoK);

	protoK = protoK * conj(eD).prime(AUXLINK,4); 
	protoK = ( protoK * delta(opPI[2],phys[2]) ) * conj(u123.H3).prime(u123.a23, 4+pl[4]);
	if(dbg && (dbgLvl >=3)) Print(protoK);
	...
	rt[0] = ITensor(prime(aux[0],pl[1]), prime(u123.a12,pl[1]), prime(aux[0],pl[1]+IOFFSET));
	rt[1] = ITensor(prime(aux[1],pl[2]), prime(u123.a12,pl[2]), prime(aux[1],pl[2]+IOFFSET));
	rt[2] = ITensor(prime(aux[1],pl[3]), prime(u123.a23,pl[3]), prime(aux[1],pl[3]+IOFFSET));
	rt[3] = ITensor(prime(aux[2],pl[4]), prime(u123.a23,pl[4]), prime(aux[2],pl[4]+IOFFSET));
	...
	// self-consistent initialization - guess isometries between tn[0] and tn[1]
		...
		// rt[0]--rt[1]
		ITensor ttemp = ( protoK * delta(prime(u123.a23,4+plRT[2]), prime(u123.a23,4+plRT[3])) )
			* delta(prime(auxRT[2],4+plRT[2]),prime(auxRT[3],4+plRT[3]));	
			
		auto cmb0 = combiner(prime(auxRT[0],4+plRT[0]), prime(u123.a12,4+plRT[0]));
		auto cmb1 = combiner(prime(auxRT[1],4+plRT[1]), prime(u123.a12,4+plRT[1]));
		...
		// rt[2]--rt[3]
		ttemp = ( protoK * delta(prime(u123.a12,4+plRT[0]), prime(u123.a12,4+plRT[1])) )
	...
	auto newT = getketT(cls.sites.at(tn[0]), u123.H1, {&rt[0],NULL}, (dbg && (dbgLvl >=3)) );
	cls.sites.at(tn[0]) = newT;
	newT = getketT(cls.sites.at(tn[1]), u123.H2, {&rt[1],&rt[2]}, (dbg && (dbgLvl >=3)) );
	cls.sites.at(tn[1]) = newT;
	newT = getketT(cls.sites.at(tn[2]), u123.H3, {&rt[3],NULL}, (dbg && (dbgLvl >=3)) );
