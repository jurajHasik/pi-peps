...
namespace itensor {
  ...
  // template<>
  void RsvdSolver::solve(MatRefc<Real> const& M,
                         MatRef<Real> const& U,
                         VectorRef const& D,
                         MatRef<Real> const& V,
                         Args const& args) {
...
#  endif
    ...
    // we never ask for computing more singular values than the rank of the
    // matrix
    ...
    // low_rank_svd_rand_decomp_fixed_rank(wM, k, p, vnum, q, s, &frank, &wU,
    // &tS, &wV);
    low_rank_svd_rand_decomp_fixed_rank_byRef(wM, k, p, vnum, q, s, &frank, &wU,
                                              &tS, &wV);
...
#  endif
  ...
  // template<>
  void RsvdSolver::solve(MatRefc<Cplx> const& M,
                         MatRef<Cplx> const& U,
                         VectorRef const& D,
                         MatRef<Cplx> const& V,
                         Args const& args) {
