...
namespace itensor {
  ...
  template <typename T>
  Spectrum svdImpl(ITensor const& A,
                   Index const& ui,
                   Index const& vi,
                   ITensor& U,
                   ITensor& D,
    ...
    if (do_truncate) {
      tie(truncerr, docut) =
        truncate(probs, maxm, minm, cutoff, absoluteCutoff, doRelCutoff, args);
      m = probs.size();
      resize(DD, m);
      reduceCols(UU, m);
      reduceCols(VV, m);
    ...
    Index uL(lname, m, litype), vL(rname, m, ritype);

    // Fix sign to make sure D has positive elements
    Real signfix = (A.scale().sign() == -1) ? -1 : +1;
    D =
      ITensor({uL, vL}, Diag<Real>{DD.begin(), DD.end()}, A.scale() * signfix);
  ...
  Spectrum svdRank2(ITensorT<IndexT> const& A,
                    ...
                    SvdSolver& solver,
                    Args args) {
    auto do_truncate = args.defined("Cutoff") || args.defined("Maxm");
  ...
  template Spectrum svdRank2(ITensor const&,
                             Index const&,
                             Index const&,
                             ITensor&,
                             ITensor&,
  ...
  template void SVDRef(MatRefc<Real> const&,
                       MatRef<Real> const&,
                       VectorRef const&,
                       MatRef<Real> const&,
                       SvdSolver& solver,
                       Args const& args);
  template void SVDRef(MatRefc<Cplx> const&,
                       MatRef<Cplx> const&,
                       VectorRef const&,
                       MatRef<Cplx> const&,
                       SvdSolver& solver,
