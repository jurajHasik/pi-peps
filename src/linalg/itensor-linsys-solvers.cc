...
namespace itensor {
  ...
  void linsystemImpl(ITensor& A,
    ...
    auto active =
      (other == A.inds().front()) ? A.inds().back() : A.inds().front();
    ...
    if (dbg) {
      ...
      std::cout << "[linsystemImpl] active: " << active << " other: " << other
                << std::endl;
  ...
  template void linsystemRank2(ITensor&,
                               ITensor&,
                               ITensor&,
                               LinSysSolver const&,
                               Args const&);

  void LinSysSolver::solve(ITensor& A,
                           ITensor& B,
                           ITensor& X,
  ...
  template void linsystemMatVec(MatRefc<Real> const&,
                                VecRef<Real> const&,
                                VecRef<Real> const&,
                                LinSysSolver const& solver,
                                Args const& args);
  template void linsystemMatVec(MatRefc<Cplx> const&,
                                VecRef<Cplx> const&,
                                VecRef<Cplx> const&,
                                LinSysSolver const& solver,
                                Args const& args);

  void PseudoInvSolver::solve(ITensor& A,
                              ITensor& B,
                              ITensor& X,
                              Args const& args) const {
    ...
    std::vector<double> elems_regInvS;
    elems_regInvS.reserve(rank);
    ...
    double const ins = (args.defined("pseudoInvCutoffInsert"))
                         ? args.getReal("pseudoInvCutoffInsert")
                         : 0.0;

    if (dbg && (dbgLvl >= 1)) {
      std::cout << "[PseudoInvSolver::solve] pinv_cutoff = " << tol
                << std::endl;
    }
