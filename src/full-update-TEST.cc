...
Args fullUpdate_COMB_INV(MPO_3site const& uJ1J2,
                         Cluster& cls,
                         CtmEnv const& ctmEnv,
  ...
  std::array<const ITensor*, 4> mpo(
    {&uJ1J2.H1, &uJ1J2.H2, &uJ1J2.H3, &dummyMPO});

  // find integer identifier of on-site tensors within CtmEnv
  std::vector<int> si;
  for (int i = 0; i <= 3; i++) {
    si.push_back(std::distance(
      ctmEnv.siteIds.begin(),
      std::find(std::begin(ctmEnv.siteIds), std::end(ctmEnv.siteIds), tn[i])));
  }
  if (dbg) {
  ...
  std::array<Index, 4> phys({noprime(findtype(cls.sites.at(tn[0]), PHYS)),
  ...
  std::vector<int> plOfSite(
    {0, 1, 2, 3});  // aux-indices (primeLevels) of on-site tensor
...

  {
    ...
    for (int s = 0; s <= 3; s++) {
      ...
      std::set_difference(plOfSite.begin(), plOfSite.end(), connected.begin(),
                          connected.end(),
  ...
  if (symmProtoEnv) {
    ...
    if (posDefProtoEnv) {
      ...
      INVD_eRE =
        diagTensor(invElems, D_eRE.inds().front(), D_eRE.inds().back());
  ...
  ITensor protoK =
    (eRE * eA) * delta(prime(aux[0], pl[1]), prime(aux[1], pl[2]));
  ...
  nEA =
    (nEA * tempSV) * delta(commonIndex(tempSV, tempT), prime(aux[0], pl[1]));
  ...
  if (otNormType == "PTN3") {
    ...
    for (int i = 0; i < 4; i++) {
      ot_norms.push_back(norm(cls.sites.at(tn[i])));
      ot_norms_tot += ot_norms.back();
    }
    for (int i = 0; i < 4; i++)
      cls.sites.at(tn[i]) =
...
ITensor psInv(ITensor const& M, Args const& args) {
  ...
  auto Msym =
    0.5 *
    (M +
     ((conj(M) * delta(i0, prime(i1, 1))) * delta(prime(i0, 1), i1)).prime(-1));
  ...
  if (dbg && (dbgLvl >= 1)) {
    ...
    for (int idm = 1; idm <= dbg_D.inds().front().m(); idm++)
      std::cout << dbg_D.real(dbg_D.inds().front()(idm),
                              dbg_D.inds().back()(idm))
                << std::endl;
  }
  ...
  for (int idm = 1; idm <= dM.inds().front().m(); idm++) {
    if (dM.real(dM.inds().front()(idm), dM.inds().back()(idm)) /
        ...
        svd_cutoff) {
      elems_regInvDM.push_back(
        msign * 1.0 / dM.real(dM.inds().front()(idm), dM.inds().back()(idm)));
    } else {
  ...
  auto regInvDM =
    diagTensor(elems_regInvDM, dM.inds().front(), dM.inds().back());

  if (dbg && (dbgLvl >= 1)) {
    std::cout << "regInvDM.scale(): " << regInvDM.scale() << std::endl;
    std::cout << "cutoff/total: " << countCTF << " / "
              << regInvDM.inds().front().m() << std::endl;
  }
...
Args fullUpdate_COMB_CG(MPO_3site const& uJ1J2,
                        Cluster& cls,
                        CtmEnv const& ctmEnv,
                        std::vector<std::string> tn,
                        std::vector<int> pl,
...
>>>>>>> amends, moving warnings.h into config
  ...
  auto tenX = (((eA * delta(prime(aux[0], pl[1]), prime(aux[1], pl[2]))) * eB) *
               delta(prime(aux[1], pl[3]), prime(aux[2], pl[4]))) *
              eD;
  ...
  auto cmbB = combiner(prime(iQA, 4), phys[0], prime(iQB, 4), phys[1],
                       prime(iQD, 4), phys[2]);
  ...
  for (int i = 1; i <= combinedIndex(cmbX).m(); i++)
    vecX[i - 1] = tenX.real(combinedIndex(cmbX)(i));
  // PrintData(tenX);
  tenX *= cmbX;

  protoK *= cmbB;
  for (int i = 1; i <= combinedIndex(cmbB).m(); i++)
    vecB[i - 1] = protoK.real(combinedIndex(cmbB)(i));
  // PrintData(protoK);
  protoK *= cmbB;
...
Funcd::Funcd(ITensor const& NN,
             ITensor const& ccmbKet,
             ITensor const& ccmbBra,
             std::vector<double> const& vvecB,
             double ffconst)
  : N(NN), cmbKet(ccmbKet), cmbBra(ccmbBra), vecB(vvecB), fconst(ffconst) {}

Doub Funcd::operator()(VecDoub_I& x) {
  auto tenX = ITensor(combinedIndex(cmbKet));
  auto tenB = ITensor(combinedIndex(cmbBra));

  for (int i = 1; i <= combinedIndex(cmbKet).m(); i++)
    tenX.set(combinedIndex(cmbKet)(i), x[i - 1]);
  tenX *= cmbKet;

  for (int i = 1; i <= combinedIndex(cmbBra).m(); i++)
    tenB.set(combinedIndex(cmbBra)(i), vecB[i - 1]);
  tenB *= cmbBra;

  auto NORM = (tenX * N) * prime(tenX, AUXLINK, 4);
  auto OVERLAP = prime(tenX, AUXLINK, 4) * tenB;

  if (NORM.r() > 0 || OVERLAP.r() > 0)
    std::cout << "Funcd() NORM or OVERLAP rank > 0" << std::endl;
...
void Funcd::df(VecDoub_I& x, VecDoub_O& deriv) {
  ...
  for (int i = 1; i <= combinedIndex(cmbBra).m(); i++)
    deriv[i - 1] = tenX.real(combinedIndex(cmbBra)(i)) - vecB[i - 1];
}

Args fullUpdate_CG(MPO_3site const& uJ1J2,
                   Cluster& cls,
                   CtmEnv const& ctmEnv,
                   std::vector<std::string> tn,
                   std::vector<int> pl,
  ...
  for (int i = 0; i < 4; i++)
    aux[i] = cls.aux[cls.SI.at(tn[i])];

  std::array<Index, 4> phys;
  for (int i = 0; i < 4; i++)
    phys[i] = cls.phys[cls.SI.at(tn[i])];
  ...
  if (symmProtoEnv) {
    ...
    if (posDefProtoEnv) {
      ...
      diag_protoEnv = std::to_string(mval) + " " + std::to_string(countCTF) +
                      " " + std::to_string(countNEG) + " " +
                      std::to_string(dM_elems.size());
      if (dbg && (dbgLvl >= 1)) {
        std::cout << "REFINED SPECTRUM" << std::endl;
...
>>>>>>> amends, moving warnings.h into config
  ...
  auto NORMPSI =
    ((eRE * (eA * delta(prime(aux[0], pl[1]), prime(aux[1], pl[2])))) *
     (eB * delta(prime(aux[1], pl[3]), prime(aux[2], pl[4])))) *
    eD;
  NORMPSI *= (prime(conj(eA), AUXLINK, 4) *
              delta(prime(aux[0], pl[1] + 4), prime(aux[1], pl[2] + 4)));
  NORMPSI *= (prime(conj(eB), AUXLINK, 4) *
              delta(prime(aux[1], pl[3] + 4), prime(aux[2], pl[4] + 4)));
  ...
  // <psi'|U|psi> = <psi|U|psi>
  auto OVERLAP =
    protoK * (prime(conj(eA), AUXLINK, 4) *
              delta(prime(aux[0], pl[1] + 4), prime(aux[1], pl[2] + 4)));
  OVERLAP *= (prime(conj(eB), AUXLINK, 4) *
              delta(prime(aux[1], pl[3] + 4), prime(aux[2], pl[4] + 4)));
  OVERLAP *= prime(conj(eD), AUXLINK, 4);

  if (NORMPSI.r() > 0 || NORMUPSI.r() > 0 || OVERLAP.r() > 0)
    std::cout << "NORMPSI or OVERLAP rank > 0" << std::endl;

  double normUPsi = sumels(NORMUPSI);

  VecDoub_IO vecX(combinedIndex(cmbX1).m() + combinedIndex(cmbX2).m() +
                  combinedIndex(cmbX3).m());
  ...
  for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
    vecX[i - 1] = eA.real(combinedIndex(cmbX1)(i));
  for (int i = 1; i <= combinedIndex(cmbX2).m(); i++)
    vecX[combinedIndex(cmbX1).m() + i - 1] = eB.real(combinedIndex(cmbX2)(i));
  for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
    vecX[combinedIndex(cmbX1).m() + combinedIndex(cmbX2).m() + i - 1] =
      eD.real(combinedIndex(cmbX3)(i));
  eA *= cmbX1;
  eB *= cmbX2;
  eD *= cmbX3;

  // double initDist = 2.0 * (1.0 - sumels(OVERLAP) / std::sqrt(normUPsi *
  // sumels(NORMPSI)) );
  ...
  t_begin_int = std::chrono::steady_clock::now();
  // FuncCG funcCG(eRE, protoK, cmbX1, cmbX2, cmbX3, aux, pl, normUPsi,
  // initDist); FrprmnV2<FuncCG> frprmn(funcCG, cg_fdistance_eps,
  // cg_gradientNorm_eps, 	cg_linesearch_eps, maxAltLstSqrIter, initDist);

  FuncCGV2 funcCG(eRE, protoK, cmbX1, cmbX2, cmbX3, aux, pl, normUPsi,
                  initDist);
  ...
  std::cout << "log_xmin: " << frprmn.log_xmin
            << " log_xmax: " << frprmn.log_xmax << std::endl;

  eA *= cmbX1;
  eB *= cmbX2;
  eD *= cmbX3;
  for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
    eA.set(combinedIndex(cmbX1)(i), vecX[i - 1]);
  for (int i = 1; i <= combinedIndex(cmbX2).m(); i++)
    eB.set(combinedIndex(cmbX2)(i), vecX[combinedIndex(cmbX1).m() + i - 1]);
  for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
    eD.set(combinedIndex(cmbX3)(i),
           vecX[combinedIndex(cmbX1).m() + combinedIndex(cmbX2).m() + i - 1]);
  eA *= cmbX1;
  eB *= cmbX2;
  eD *= cmbX3;
  ...
  oss << std::scientific << " " << locMinData.final_f << " "
      << locMinData.final_g2 << " " << funcCG.evalCount << " "
      << funcCG.dfCount;
...
FuncCG::FuncCG(ITensor const& NN,
               ITensor const& pprotoK,
               ITensor const& ccmbX1,
               ITensor const& ccmbX2,
               ITensor const& ccmbX3,
               std::array<Index, 4> const& aaux,
...
Doub FuncCG::operator()(VecDoub_I& x) {
  ...
  for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
    eA.set(combinedIndex(cmbX1)(i), x[i - 1]);
  for (int i = 1; i <= combinedIndex(cmbX2).m(); i++)
    eB.set(combinedIndex(cmbX2)(i), x[combinedIndex(cmbX1).m() + i - 1]);
  for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
    eD.set(combinedIndex(cmbX3)(i),
           x[combinedIndex(cmbX1).m() + combinedIndex(cmbX2).m() + i - 1]);
  eA *= cmbX1;
  eB *= cmbX2;
  eD *= cmbX3;
  ...
  auto NORM = ((N * (eA * delta(prime(aux[0], pl[1]), prime(aux[1], pl[2])))) *
               (eB * delta(prime(aux[1], pl[3]), prime(aux[2], pl[4])))) *
              eD;
  NORM *= (prime(conj(eA), AUXLINK, 4) *
           delta(prime(aux[0], pl[1] + 4), prime(aux[1], pl[2] + 4)));
  NORM *= (prime(conj(eB), AUXLINK, 4) *
           delta(prime(aux[1], pl[3] + 4), prime(aux[2], pl[4] + 4)));
  ...
  psiNorm = sumels(NORM);
  // return 2.0 * ( 1.0 - sumels(OVERLAP)/std::sqrt(psiUNorm * psiNorm) ) /
  // finit;
...
void FuncCG::df(VecDoub_I& x, VecDoub_O& deriv) {
  ...
  auto protoM =
    ((N * (eA * delta(prime(aux[0], pl[1]), prime(aux[1], pl[2])))) *
     (eB * delta(prime(aux[1], pl[3]), prime(aux[2], pl[4])))) *
    eD;

  // compute eA part of gradient
  auto M = protoM * (prime(conj(eD), AUXLINK, 4) *
                     delta(prime(aux[2], pl[4] + 4), prime(aux[1], pl[3] + 4)));
  M *= (prime(conj(eB), AUXLINK, 4) *
        delta(prime(aux[1], pl[2] + 4), prime(aux[0], pl[1] + 4)));

  auto K = protoK * (prime(conj(eD), AUXLINK, 4) *
                     delta(prime(aux[2], pl[4] + 4), prime(aux[1], pl[3] + 4)));
  ...
  for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
    deriv[i - 1] =
      M.real(combinedIndex(cmbX1)(i)) - K.real(combinedIndex(cmbX1)(i));

  // compute eB part of gradient
  M = protoM * (prime(conj(eD), AUXLINK, 4) *
                delta(prime(aux[2], pl[4] + 4), prime(aux[1], pl[3] + 4)));
  ...
  for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
    deriv[combinedIndex(cmbX1).m() + combinedIndex(cmbX2).m() + i - 1] =
      M.real(combinedIndex(cmbX3)(i)) - K.real(combinedIndex(cmbX3)(i));
}

//-----------------------------------------------------------------------------
FuncCGV2::FuncCGV2(ITensor const& NN,
                   ITensor const& pprotoK,
                   ITensor const& ccmbX1,
                   ITensor const& ccmbX2,
                   ITensor const& ccmbX3,
...
Args fullUpdate_ALS_CG(MPO_3site const& uJ1J2,
                       Cluster& cls,
                       CtmEnv const& ctmEnv,
                       std::vector<std::string> tn,
                       std::vector<int> pl,
...
>>>>>>> amends, moving warnings.h into config
  ...
  FrprmnALSCG<FuncALS_CG> frprmn(funcALS_CG, cg_fdistance_eps,
                                 cg_gradientNorm_eps, cg_linesearch_eps,
                                 maxAltLstSqrIter, 0.0);
  ...
  while (not converged) {
    ...
    if (fdist.back() < iso_eps) {
      converged = true;
      break;
    }
    if ((fdist.size() > 1) &&
        std::abs(fdist.back() - fdist[fdist.size() - 2]) < iso_eps) {
      converged = true;
      break;
    }
    ...
    eA *= cmbX1;
    for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
      vecX1[i - 1] = eA.real(combinedIndex(cmbX1)(i));

    std::cout << "f_init= " << finit << std::endl;
    funcALS_CG.setup(cmbX1, normUPsi, finit, normPsi);
    frprmn.setup(finit);
    // Frprmn<FuncALS_CG> frprmn(funcALS_CG, iso_eps, iso_eps,
    // maxAltLstSqrIter);
    ...
    for (int i = 1; i <= combinedIndex(cmbX1).m(); i++)
      tempEA.set(combinedIndex(cmbX1)(i), vecX1[i - 1]);
    eA *= cmbX1;
    tempEA *= cmbX1;
    ...
    eB *= cmbX2;
    for (int i = 1; i <= combinedIndex(cmbX2).m(); i++)
      vecX2[i - 1] = eB.real(combinedIndex(cmbX2)(i));
    ...
    for (int i = 1; i <= combinedIndex(cmbX2).m(); i++)
      tempEB.set(combinedIndex(cmbX2)(i), vecX2[i - 1]);
    eB *= cmbX2;
    tempEB *= cmbX2;
    ...
    eD *= cmbX3;
    for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
      vecX1[i - 1] = eD.real(combinedIndex(cmbX3)(i));
    ...
    for (int i = 1; i <= combinedIndex(cmbX3).m(); i++)
      tempED.set(combinedIndex(cmbX3)(i), vecX1[i - 1]);
    eD *= cmbX3;
    tempED *= cmbX3;
  ...
  for (int i = 0; i < fdist.size(); i++)
    std::cout << "STEP " << i << "||psi'>-|psi>|^2: " << fdist[i] << std::endl;
  ...
  // 	std::vector<double> ot_norms;
  // 	for (int i=0; i<4; i++)
  // 		{ ot_norms.push_back(norm(cls.sites.at(tn[i]))); ot_norms_tot +=
  // ot_norms.back(); } 	for (int i=0; i<4; i++) cls.sites.at(tn[i]) =
  // 		cls.sites.at(tn[i]) / std::pow(nn, (ot_norms[i]/ot_norms_tot));
  // } else if (otNormType == "BLE") {
  if (otNormType == "BLE") {
    for (int i = 0; i < 3; i++) {
...
FuncALS_CG::FuncALS_CG(ITensor const& MM,
                       ITensor& KK,
                       ITensor ccmbKet,
                       double ppsiUNorm,
                       double ffinit,
...
void FuncALS_CG::setup(ITensor ccmbKet,
                       double ppsiUNorm,
                       double ffinit,
                       double ppsiNorm) {
  cmbKet = ccmbKet;
...
Doub FuncALS_CG::operator()(VecDoub_I& x) {
  ...
  for (int i = 1; i <= combinedIndex(cmbKet).m(); i++)
    tmpX.set(combinedIndex(cmbKet)(i), x[i - 1]);
  tmpX *= cmbKet;
...
void FuncALS_CG::df(VecDoub_I& x, VecDoub_O& deriv) {
  ...
  for (int i = 1; i <= combinedIndex(cmbKet).m(); i++)
    deriv[i - 1] =
      tmpX.real(combinedIndex(cmbKet)(i)) - K.real(combinedIndex(cmbKet)(i));
...
//     cls.sites.at(tn[1]), 	cls.sites.at(tn[2]) };
...
// 3))); 		tempC = pc[3] * cls.sites.at(tn[3]); 		tempC *=
// prime(conj(cls.sites.at(tn[3])), AUXLINK, 4); 		if(dbg && (dbgLvl >=3))
// Print(tempC);
...
// 					msign = dM_elems.back()/mval;
// 				}
// 				if (std::abs(dM_elems.back()) < nval) nval =
// std::abs(dM_elems.back());
// 			}
// 			if (msign < 0.0) for (auto & elem : dM_elems) elem = elem*(-1.0);
...
// : 1.0/svd_cutoff ; 			condNum = mval / nval;
...
// 	std::cout<<"eRE.scale(): "<< eRE.scale()<<" protoK.scale(): "<<
// protoK.scale() <<std::endl; 	t_end_int = std::chrono::steady_clock::now();
// 	std::cout<<"Proto Envs for M and K constructed - T: "<<
// 		std::chrono::duration_cast<std::chrono::microseconds>(t_end_int -
// t_begin_int).count()/1000000.0 <<" [sec]"<<std::endl;
// 	// ***** FORM "PROTO" ENVIRONMENTS FOR M and K DONE ************************
...
// 	// <psi|U^dag U|psi>
// 	auto NORMUPSI = (( protoK * delta(opPI[0],phys[0]) ) * conj(uJ1J2.H1)) *
// prime(delta(opPI[0],phys[0])); 	NORMUPSI = (( NORMUPSI *
// delta(opPI[1],phys[1]) ) * conj(uJ1J2.H2)) * prime(delta(opPI[1],phys[1]));
// 	NORMUPSI = (( NORMUPSI * delta(opPI[2],phys[2]) ) * conj(uJ1J2.H3)) *
// prime(delta(opPI[2],phys[2])); 	NORMUPSI.prime(PHYS,-1); 	NORMUPSI *=
// (prime(conj(eA), AUXLINK, 4) *
// delta(prime(aux[0],pl[1]+4),prime(aux[1],pl[2]+4)) ); 	NORMUPSI *=
// (prime(conj(eB), AUXLINK, 4) *
// delta(prime(aux[1],pl[3]+4),prime(aux[2],pl[4]+4)) ); 	NORMUPSI *=
// prime(conj(eD), AUXLINK, 4);
...
// 		};
// 		auto sumSquares = [&extractDiagReal](ITensor const& t1, ITensor const&
// t2)->Real {
// 			// assume equal length
...
// delta(prime(opPI[0]),phys[0]); 		auto tEB = (eB * (delta(opPI[1],phys[1]) *
// uJ1J2.H2)) * delta(prime(opPI[1]),phys[1]); 		auto tED = (eD *
// (delta(opPI[2],phys[2]) * uJ1J2.H3)) * delta(prime(opPI[2]),phys[2]);
...
// 	auto cmbX1 = combiner(eA.inds()[0], eA.inds()[1], eA.inds()[2]);
// 	auto cmbX2 = combiner(eB.inds()[0], eB.inds()[1], eB.inds()[2],
// eB.inds()[3]); 	auto cmbX3 = combiner(eD.inds()[0], eD.inds()[1],
// eD.inds()[2]);
...
//   <<std::endl; 	t_begin_int = std::chrono::steady_clock::now();
// 	while (not converged) {
// 		// Optimizing eA
// 		// 1) construct matrix M, which is defined as <psi~|psi~> = eA^dag * M *
// eA
...
// 	    // Optimizing eB
// 		// 1) construct matrix M, which is defined as <psi~|psi~> = eB^dag * M *
// eB
...
// 		// Optimizing eD
// 		// 1) construct matrix M, which is defined as <psi~|psi~> = eD^dag * M *
// eD
...
// 	std::string siteMaxElem_descriptor = "site max_elem site max_elem site
// max_elem site max_elem";
// 	diag_data.add("siteMaxElem_descriptor",siteMaxElem_descriptor);
// 	diag_data.add("siteMaxElem",diag_maxElem);
// 	diag_data.add("ratioNonSymLE",diag_maxMasymLE/diag_maxMsymLE); // ratio of
// largest elements
// 	diag_data.add("ratioNonSymFN",diag_maxMasymFN/diag_maxMsymFN); // ratio of
// norms

// 	std::ostringstream oss;
// 	oss << std::scientific << fdist[0] <<" "<< fdist.back() <<" "
// 		<< fdistN[0] <<" "<< fdistN.back() <<" "<< vec_normPsi[0] <<" "<<
// vec_normPsi.back() <<" "<<
// 		std::chrono::duration_cast<std::chrono::microseconds>(t_end_int -
// t_begin_int).count()/1000000.0 ;

// 	std::string logMinDiag_descriptor = "f_init f_final normalizedf_init
// normalizedf_final norm(psi')_init norm(psi')_final time[s]";
...
FULSCG_IT::FULSCG_IT(ITensor& MM,
                     ITensor& BB,
                     ITensor& AA,
                     ITensor ccmbA,
                     ITensor ccmbKet,
...
void FULSCG_IT::asolve(ITensor const& b, ITensor& x, const Int itrnsp) {
  ...
  // 	double elem = M.real(combinedIndex(cmbKet)(i+1),
  // prime(combinedIndex(cmbKet),4)(i+1)); 	mval = std::max(mval,
  // std::abs(elem));
  // }
  // for (int i=0; i<combinedIndex(cmbKet).size(); i++) {
  // 	double elem = M.real(combinedIndex(cmbKet)(i+1),
  // prime(combinedIndex(cmbKet),4)(i+1)); 	diagMvec[i] = (elem/mval >
  // svd_cutoff) ? 1.0/elem : 0.0 ;
  // }

  // ITensor diagM = diagTensor(diagMvec, combinedIndex(cmbKet),
  // prime(combinedIndex(cmbKet),4));
...
void FULSCG_IT::asolve_pinv(ITensor const& b, ITensor& x) {
  ...
  //  pseudoInvCutoffInsert (default= 0.0): sing.values lower than
  //  pseudoInvCutoff
...
void FULSCG_IT::solveBiCG(ITensor const& b,
                          ITensor& x,
                          const Int itol,
                          const Doub tol,
                          const Int itmax,
  ...
  // Int j,n=b.size();
  ITensor p(x.inds()), pp(x.inds()), r(x.inds()), rr(x.inds()), z(x.inds()),
    zz(x.inds());
  ...
  while (iter < itmax) {
    ...
    // if( isComplex(tempZRR) ) std::cout << "W: z*rr is complex val=" <<
    // sumelsC(tempZRR).imag() << std::endl;
    ...
    // 	std::cout<< "W: z*pp is complex val=" << sumelsC(tempZPP).imag() <<
    // std::endl;
    // }
...
void FULSCG_IT::solve(itensor::ITensor const& b,
                      itensor::ITensor& x,
                      Int& iter,
                      Doub& err,
                      Args const& args) {
  if (solver == "pseudoinverse") {
    ...
    std::cout << "[FULSCG_IT::solve] Unsupported solver: " << solver
              << std::endl;
    exit(EXIT_FAILURE);
  }
}
...
void FULSCG::asolve(VecDoub_I& b, VecDoub_O& x, const Int itrnsp) {
  ...
  for (int i = 0; i < b.size(); i++)
    tmpB.set(combinedIndex(cmbA)(i + 1), b[i]);
  ...
  for (int i = 0; i < combinedIndex(cmbKet).size(); i++) {
    double elem = M.real(combinedIndex(cmbKet)(i + 1),
                         prime(combinedIndex(cmbKet), 4)(i + 1));
  ...
  for (int i = 0; i < combinedIndex(cmbA).size(); i++)
    x[i] = tmp.real(combinedIndex(cmbA)(i + 1));
}
...
void FULSCG::atimes(VecDoub_I& x, VecDoub_O& r, const Int itrnsp) {
  ...
  for (int i = 0; i < x.size(); i++)
    tmpA.set(combinedIndex(cmbA)(i + 1), x[i]);
  tmpA *= cmbA;
  ...
  for (int i = 0; i < x.size(); i++)
    r[i] = tmp.real(combinedIndex(cmbA)(i + 1));
}

//-----------------------------------------------------------------------------

void Linbcg::solve(VecDoub_I& b,
                   VecDoub_IO& x,
                   const Int itol,
                   const Doub tol,
                   const Int itmax,
...
// 		temp = ((temp * delta(opPI[0],phys[0])) * delta(opPI[1],phys[1])) *
// delta(opPI[2],phys[2]);
...
// 			* delta(prime(opPI[2]),phys[2]);
// 	}
// 	// TODO For now, assume the operator does not act on 4th site - just a
// syntactic filler
...
// prime(delta(opPI[2],phys[2]));
// 	// TODO For now, assume the operator does not act on 4th site - just a
// syntactic filler 	NORMUPSI = NORMUPSI * delta(phys[3], prime(phys[3]));
...
// 	for (int i=0; i<4; i++) {
// 		pcS[i] = (pc[i] * cls.sites.at(tn[i])) * prime(conj(cls.sites.at(tn[i])),
// AUXLINK,4);
// 	}
// 	double normPsi, finit, finitN;
// 	double prev_finit = 1.0;
...
//   	int altlstsquares_iter = 0;
// 	bool converged = false;
// 	// cg_gradientNorm_eps = std::max(cg_gradientNorm_eps, condNum *
// machine_eps);
// 	// // cg_fdistance_eps    = std::max(cg_fdistance_eps, condNum *
// machine_eps);
//   	std::vector<double> fdist, fdistN, vec_normPsi;
...
// 		// M = (pc[1] * cls.sites.at(tn[1])) * prime(conj(cls.sites.at(tn[1])),
// AUXLINK,4);
// 		// M *= delta(prime(aux[1],pl[3]),  prime(aux[2],pl[4]));
// 		// M *= delta(prime(aux[1],pl[3]+4),prime(aux[2],pl[4]+4));
// 		// M = ((M * pc[2]) * cls.sites.at(tn[2])) *
// prime(conj(cls.sites.at(tn[2])), AUXLINK,4);
// 		// M *= delta(prime(aux[2],pl[5]),  prime(aux[3],pl[6]));
// 		// M *= delta(prime(aux[2],pl[5]+4),prime(aux[3],pl[6]+4));
// 		// M = ((M * pc[3]) * cls.sites.at(tn[3])) *
// prime(conj(cls.sites.at(tn[3])), AUXLINK,4);
...
// 		// <psi'|psi'>
// 		NORMPSI = (prime(conj(cls.sites.at(tn[0])), AUXLINK,4) * M) *
// cls.sites.at(tn[0]);
// 		// <psi'|U|psi>
...
// 		if ( (fdist.size() > 1) && std::abs((fdist.back() -
...
// 		// M = (pc[2] * cls.sites.at(tn[2])) * prime(conj(cls.sites.at(tn[2])),
// AUXLINK,4);
// 		// M *= delta(prime(aux[2],pl[5]),  prime(aux[3],pl[6]));
// 		// M *= delta(prime(aux[2],pl[5]+4),prime(aux[3],pl[6]+4));
// 		// M = ((M * pc[3]) * cls.sites.at(tn[3])) *
...
// 		// M *= delta(prime(aux[3],pl[7]),  prime(aux[0],pl[0]));
// 		// M *= delta(prime(aux[3],pl[7]+4),prime(aux[0],pl[0]+4));
// 		// M = ((M * pc[0]) * cls.sites.at(tn[0])) *
// prime(conj(cls.sites.at(tn[0])), AUXLINK,4);
// 		// M *= delta(prime(aux[0],pl[1]),  prime(aux[1],pl[2]));
// 		// M *= delta(prime(aux[0],pl[1]+4),prime(aux[1],pl[2]+4));
// 		// M *= delta(prime(aux[2],pl[4]),  prime(aux[1],pl[3]));
// 		// M *= delta(prime(aux[2],pl[4]+4),prime(aux[1],pl[3]+4));
...
// 		// <psi'|psi'>
// 		NORMPSI = (prime(conj(cls.sites.at(tn[1])), AUXLINK,4) * M) *
// cls.sites.at(tn[1]);
// 		// <psi'|U|psi>
...
// 		// M = (pc[3] * cls.sites.at(tn[3])) * prime(conj(cls.sites.at(tn[3])),
// AUXLINK,4);
// 		// M *= delta(prime(aux[3],pl[7]),  prime(aux[0],pl[0]));
// 		// M *= delta(prime(aux[3],pl[7]+4),prime(aux[0],pl[0]+4));
// 		// M = ((M * pc[0]) * cls.sites.at(tn[0])) *
...
// 		// M *= delta(prime(aux[0],pl[1]),  prime(aux[1],pl[2]));
// 		// M *= delta(prime(aux[0],pl[1]+4),prime(aux[1],pl[2]+4));
// 		// M = ((M * pc[1]) * cls.sites.at(tn[1])) *
// prime(conj(cls.sites.at(tn[1])), AUXLINK,4);
// 		// M *= delta(prime(aux[1],pl[3]),  prime(aux[2],pl[4]));
// 		// M *= delta(prime(aux[1],pl[3]+4),prime(aux[2],pl[4]+4));
...
// 		// <psi'|psi'>
// 		NORMPSI = (prime(conj(cls.sites.at(tn[2])), AUXLINK,4) * M) *
// cls.sites.at(tn[2]);
// 		// <psi'|U|psi>
...
// 		pcS[2] = (pc[2] * cls.sites.at(tn[2])) * prime(conj(cls.sites.at(tn[2])),
// AUXLINK,4);

// 		// Optimizing C
// 		// 1) construct matrix M, which is defined as <psi~|psi~> = C^dag * M * C

// 		// M = (pc[0] * cls.sites.at(tn[0])) * prime(conj(cls.sites.at(tn[0])),
// AUXLINK,4);
// 		// M *= delta(prime(aux[0],pl[1]),  prime(aux[1],pl[2]));
// 		// M *= delta(prime(aux[0],pl[1]+4),prime(aux[1],pl[2]+4));
// 		// M = ((M * pc[1]) * cls.sites.at(tn[1])) *
...
// 		// <psi'|psi'>
// 		NORMPSI = (prime(conj(cls.sites.at(tn[3])), AUXLINK,4) * M) *
// cls.sites.at(tn[3]);
// 		// <psi'|U|psi>
...
// 		pcS[3] = (pc[3] * cls.sites.at(tn[3])) * prime(conj(cls.sites.at(tn[3])),
// AUXLINK,4);
...
// 	// diag_data.add("ratioNonSymLE",diag_maxMasymLE/diag_maxMsymLE); // ratio
// of largest elements
// 	// diag_data.add("ratioNonSymFN",diag_maxMasymFN/diag_maxMsymFN); // ratio
// of norms
...
// ***** PRODUCTION LEVEL ALS over 4 sites applying 4 site operator, BiCG with
// ITensor
Args fullUpdate_ALS4S_LSCG_IT(OpNS const& uJ1J2,
                              Cluster& cls,
                              CtmEnv const& ctmEnv,
                              std::vector<std::string> tn,
                              std::vector<int> pl,
  ...
  std::array<Index, 4> opPI({noprime(uJ1J2.pi[0]), noprime(uJ1J2.pi[1]),
                             noprime(uJ1J2.pi[2]), noprime(uJ1J2.pi[3])});
  ...
  std::vector<ITensor> pc(4);  // holds corners T-C-T
  {
    ...
    for (int s = 0; s <= 3; s++) {
      ...
      // tensors define combiner
      auto cmb0 =
        combiner(prime(aux[s], tmp_iToE[0]), prime(aux[s], tmp_iToE[0] + 4));
      auto cmb1 =
        combiner(prime(aux[s], tmp_iToE[1]), prime(aux[s], tmp_iToE[1] + 4));
      if (dbg && dbgLvl >= 3) {
        Print(cmb0);
        Print(cmb1);
      }
      ...
      pc[s] =
        (pc[s] * delta(combinedIndex(cmb0), iToE[s][tmp_iToE[0]]) * cmb0) *
        delta(combinedIndex(cmb1), iToE[s][tmp_iToE[1]]) * cmb1;
    }
    if (dbg) {
      for (int i = 0; i <= 3; i++) {
  ...
  for (int i = 0; i < 4; i++) {
    ITensor X(prime(aux[i], pl[2 * i]), prime(aux[i], pl[2 * i + 1]), phys[i]),
      Q, SV;
  ...
  for (int i = 0; i < 4; i++) {
    pc[i] *= qX[i];
    pc[i] *= prime(conj(qX[i]), AUXLINK, 4);
  }
  ...
  if (NORMUPSI.r() > 0)
    std::cout << "NORMUPSI rank > 0" << std::endl;
  double normUPsi = sumels(NORMUPSI);
  ...
  std::cout << "ENTERING ALS LOOP tol: " << epsdistf << " solver: " << solver
            << std::endl;
  t_begin_int = std::chrono::steady_clock::now();
  while (not converged) {
    // ***** Optimizing A *************************************************
    ...
    finitN = prev_finitN =
      1.0 - 2.0 * sumels(OVERLAP) / std::sqrt(normUPsi * normPsi) + 1.0;
    ...
    std::cout << "stopCond: "
              << (fdist.back() - fdist[fdist.size() - 2]) / fdist[0]
              << " ";  // std::endl;
    // if ( (fdist.size() > 1) && std::abs((fdist.back() -
    // fdist[fdist.size()-2])/fdist[0]) < epsdistf ) if ( (fdist.size() > 1) &&
    // (std::abs( fdistN.back() - fdistN[fdistN.size()-2] ) < epsdistf || finitN
    // < epsdistf ) )
    if ((fdist.size() > 1) &&
        (std::abs(fdistN.back() - fdistN[fdistN.size() - 2]) < epsdistf)) {
      converged = true;
      break;
    }
    if (fdistN_growth[0] && fdistN_growth[1] && fdistN_growth[2] &&
        fdistN_growth[3]) {
      converged = true;
      break;
    }
    ...
    std::cout << "Norm(init): " << norm(oldSite)
              << " Norm(final): " << norm(rX[0]) << std::endl;
    // rX[0] /= norm(rX[0]);
    ...
    std::cout << "stopCond: "
              ...
              << " ";  // std::endl;
    // if ( (fdist.size() > 1) && std::abs((fdist.back() -
    // fdist[fdist.size()-2])/fdist[0]) < epsdistf )
    // if ( (fdist.size() > 1) && (std::abs( fdistN.back() -
    // fdistN[fdistN.size()-2] ) < epsdistf || finitN < epsdistf ) )
    if ((fdist.size() > 1) &&
        (std::abs(fdistN.back() - fdistN[fdistN.size() - 2]) < epsdistf)) {
      converged = true;
      break;
