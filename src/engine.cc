...
//         pe->td.gateMPO.push_back( getMPO3s_Ising3Body(arg_tau, arg_J1,
//         arg_J2, arg_h) ); pe->td.ptr_gateMPO = std::vector< MPO_3site* >(16,
//         &pe->td.gateMPO[0] );
...
//     return simpleUpdate(*td.ptr_gateMPO[gi], cls, td.gates[gi],
...
template <>
Args TrotterEngine<MPO_3site>::performSimpleUpdate(Cluster& cls,
                                                   Args const& args) {
  auto gi = td.nextCyclicIndex();
...
template <>
Args TrotterEngine<OpNS>::performSimpleUpdate(Cluster& cls, Args const& args) {
  auto gi = td.nextCyclicIndex();
...
template <class T>
Args TrotterEngine<T>::performFullUpdate(Cluster& cls,
                                         CtmEnv const& ctmEnv,
                                         Args const& args) {
  return Args::global();
}
...
template <>
Args TrotterEngine<MPO_2site>::performFullUpdate(Cluster& cls,
                                                 CtmEnv const& ctmEnv,
                                                 Args const& args) {
  auto gi = td.nextCyclicIndex();
  ...
  return fullUpdate_ALS2S_IT(*td.tgates[gi].ptr_gate, cls, ctmEnv,
                             ...
                             *(this->pSolver), args);
  // NEW_INTERFACE return fullUpdate_ALS2S_IT(tgates[gi], cls, ctmEnv,
  // *(this->pSolver), args);
...
template <>
Args TrotterEngine<MPO_3site>::performFullUpdate(Cluster& cls,
                                                 CtmEnv const& ctmEnv,
                                                 Args const& args) {
  auto gi = td.nextCyclicIndex();
  ...
  return fullUpdate_ALS3S_IT(*td.tgates[gi].ptr_gate, cls, ctmEnv,
                             tmp_siteId_seq, tmp_auxIndsDir_seq,
                             *(this->pSolver), args);
  // NEW_INTERFACE return fullUpdate_ALS3S_IT(tgates[gi], cls, ctmEnv,
  // *(this->pSolver), args);
}

template <>
Args TrotterEngine<OpNS>::performFullUpdate(Cluster& cls,
                                            CtmEnv const& ctmEnv,
                                            Args const& args) {
  auto gi = td.nextCyclicIndex();

  if (td.tgates[gi].ptr_gate->nSite != 4) {
    std::cout << "Unsupported OpNS: nSite = " << td.tgates[gi].ptr_gate->nSite
              << std::endl;
    exit(EXIT_FAILURE);
  }
