...
namespace itensor {
  ...
  TEST(ArpackRealSvd0, Default_cotr) {
    ...
    // true singular values of A = {21.5538, 12.5376, 9.5126,
    // 8.4900, 1.759784917702199}
    double n = norm(A - U * D * Vt);
    EXPECT_TRUE(
      std::sqrt(std::abs(n * n - 1.759784917702199 * 1.759784917702199)) < eps);
  }
  ...
  TEST(ArpackRealSvd1, Default_cotr) {
    ...
    double n = norm(A - U * D * Vt);
    EXPECT_TRUE(
      std::sqrt(std::abs(n * n - 0.772869635673485 * 0.772869635673485)) < eps);
  }
